...
- [ ] Phase 4 route acceptance passed before installer creation: regular NKN 64 MiB uses `regular_nkn_v4_fast` protocol `4`; active Tuna 64 MiB uses `file_tuna_v4` protocol `4`; live switch-off from each side ends on `post_tuna_fallback_v6` protocol `6`; live off/on/off proves `post_tuna_fallback_v6,file_tuna_v4,post_tuna_fallback_v6`; second transfer after live reactivation selects `file_tuna_v4` protocol `4`; all scenarios have SHA/integrity OK, completed terminals, hard failures `0`, no active `file_tuna_v6`, and strict route proof. Persistent goodput misses after the paired rerun are performance acceptance failures and must not be fixed by bridge tuning without a separate baseline decision.
