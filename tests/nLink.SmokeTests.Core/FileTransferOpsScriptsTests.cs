...
public sealed class FileTransferOpsScriptsTests
{
    ...
    [Fact]
    public void GuiSmoke_NknDirectConnectPreflightDoesNotDependOnSendButtonEnabledState()
    {
        var repoRoot = FindRepoRoot();
        var scriptPath = Path.Combine(repoRoot, "tools", "GuiSmoke-Windows.ps1");
        var scriptText = File.ReadAllText(scriptPath);
        var functionStart = scriptText.IndexOf("function Run-ScenarioNknDirectConnect", StringComparison.Ordinal);
        Assert.True(functionStart >= 0, "Expected NKN direct-connect scenario to exist.");
        var functionEnd = scriptText.IndexOf("function Run-ScenarioHeaderChatCoherence", functionStart, StringComparison.Ordinal);
        Assert.True(functionEnd > functionStart, "Expected to locate the end of the NKN direct-connect scenario.");

        var functionBody = scriptText[functionStart..functionEnd];
        Assert.Contains("Send-ChatMessage -Window $Context.HelperWindow -Text $msg -UseEnter", functionBody, StringComparison.Ordinal);
    }
    ...
    public void RunFileTransferNknSoak_PublicParameterSetAndArtifactNamesRemainStable()
    {
        ...
        Assert.Contains("Set-ProcessEnvironmentValue -Name 'NLINK_TRANSPORT' -Value 'NKN'", scriptText, StringComparison.Ordinal);
        foreach (var artifactName in RequiredLiveNknArtifactFiles)
        {
            Assert.Contains(artifactName, scriptText, StringComparison.Ordinal);
        }
    ...
    [Fact]
    public void GuiSmoke_FileTransferNknScenariosRequireNknTransport()
    {
        var repoRoot = FindRepoRoot();
        var scriptPath = Path.Combine(repoRoot, "tools", "GuiSmoke-Windows.ps1");
        var scriptText = File.ReadAllText(scriptPath);

        Assert.Contains("Assert-GuiSmokeNknTransport -ScenarioName 'FILETRANSFER_NKN_SOAK'", scriptText, StringComparison.Ordinal);
        Assert.Contains("Assert-GuiSmokeNknTransport -ScenarioName 'FILETRANSFER_NKN_MIXED_SOAK'", scriptText, StringComparison.Ordinal);
        Assert.Contains("requires NLINK_TRANSPORT=NKN", scriptText, StringComparison.Ordinal);
    }
    ...
    public void RunFileTransferRouteAcceptance_PublicParametersAndArtifactsRemainStable()
    {
        ...
        Assert.Contains("Ensure-RouteAcceptancePackagedBridgeRuntime", scriptText, StringComparison.Ordinal);
        Assert.Contains("Assert-RouteAcceptanceBridgeBundleRuntime", scriptText, StringComparison.Ordinal);
        Assert.Contains("bridge-runtime-{0}.txt", scriptText, StringComparison.Ordinal);
        Assert.Contains("Route acceptance app is missing the packaged NKN bridge runtime", scriptText, StringComparison.Ordinal);
        ...
        Assert.Contains("Assert-RouteAcceptanceRegularNknTransportEvidence", scriptText, StringComparison.Ordinal);
        Assert.Contains("regular NKN artifact used DevLocal transport", scriptText, StringComparison.Ordinal);
        Assert.Contains("regular NKN artifact lacks positive NKN transport evidence", scriptText, StringComparison.Ordinal);
    ...
    public void RunFileTransferTunaGuiSmoke_PreactivationAcceptFailuresAreClassified()
    {
        ...
        Assert.Contains("normal_to_tuna_live_epoch_missing", scriptText, StringComparison.Ordinal);
        Assert.Contains("normal_to_tuna_live_epoch_recovery_missing", scriptText, StringComparison.Ordinal);
        Assert.Contains("normal_to_tuna_route_selection_missing", scriptText, StringComparison.Ordinal);
    ...
    [Fact]
    public void GuiSmokeNknBootstrap_RetriesHelpeeEntryModeAfterTransientConnectionFailure()
    {
        var repoRoot = FindRepoRoot();
        var scriptPath = Path.Combine(repoRoot, "tools", "GuiSmoke-Windows.ps1");
        var scriptText = File.ReadAllText(scriptPath);

        Assert.Contains("Wait-HelpeeConnectionEntryModeWithRecovery", scriptText, StringComparison.Ordinal);
        Assert.Contains("Recovering helpee connection entry-mode wait after transient failure", scriptText, StringComparison.Ordinal);
        Assert.Contains("Reenter-RoleFlowAfterConnectionFailure -Window $Context.HelpeeWindow -HomeButtonText 'I need help'", scriptText, StringComparison.Ordinal);
        Assert.Contains("Restart-HelpeeFlow -Context $Context", scriptText, StringComparison.Ordinal);
    }
    ...
    public void RunFileTransferTunaGuiSmoke_LiveSwitchOffUsesSameTransferV6FallbackProof()
    {
        ...
        Assert.Contains("Resolve-TunaGuiLiveSwitchOffMinimumPeerVisiblePayloadBytes", guiText, StringComparison.Ordinal);
        Assert.Contains("NLINK_TUNA_GUI_LIVE_SWITCH_OFF_MIN_PEER_VISIBLE_PAYLOAD_BYTES", guiText, StringComparison.Ordinal);
        Assert.Contains("filetransfer_tuna_gui_live_switch_off_progress_gate_satisfied", guiText, StringComparison.Ordinal);
        Assert.Contains("mode=peer_visible_payload", guiText, StringComparison.Ordinal);
        Assert.Contains("-MinimumPeerVisiblePayloadBytes $liveSwitchOffMinimumPeerVisiblePayloadBytes", guiText, StringComparison.Ordinal);
    ...
    public void RunFileTransferTunaGuiSmoke_LiveMultiToggleUsesRouteEpochSequence()
    {
        ...
        Assert.Contains("$fallbackRouteLine = [string](Wait-AppLogContainsAnyAllAfterBookmark -Bookmark $stepBookmark -NeedleSets @(", guiText, StringComparison.Ordinal);
        Assert.Contains("@('event=filetransfer_route_selected', 'route=post_tuna_fallback_v6', 'protocol_version=6')", guiText, StringComparison.Ordinal);
        Assert.Contains("$activationRouteLine = [string](Wait-AppLogContainsAnyAllAfterBookmark -Bookmark $stepBookmark -NeedleSets @(", guiText, StringComparison.Ordinal);
        Assert.Contains("@('event=filetransfer_route_selected', 'route=file_tuna_v4', 'protocol_version=4')", guiText, StringComparison.Ordinal);
        Assert.DoesNotContain("$fallbackRouteLine = if ($fallbackStartedLine.IndexOf('route=post_tuna_fallback_v6'", guiText, StringComparison.Ordinal);
        Assert.DoesNotContain("$activationRouteLine = if ($activationStartedLine.IndexOf('route=file_tuna_v4'", guiText, StringComparison.Ordinal);
    ...
    public void RunFileTransferTunaGuiSmoke_LiveRegularActivationCycleStartsRegularThenCyclesRoutes()
    {
        ...
        Assert.Contains("[string]$AfterLine = ''", guiText, StringComparison.Ordinal);
        Assert.Contains("-AfterLine $AfterLine", guiText, StringComparison.Ordinal);
        ...
        Assert.Contains("-AfterLine $activationStartedLine", guiText, StringComparison.Ordinal);
        Assert.Contains("-AfterLine $fallbackStartedLine", guiText, StringComparison.Ordinal);
    ...
    public async Task AnalyzeRetained_LiveFallbackLegHistory_ReturnsRouteConsistencyPass()
    {
        ...
        var lines = new[]
        {
            ...
            LogLine($"event=filetransfer_clean_fallback_leg_frozen; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:1; leg_generation=1; route=file_tuna_v4; protocol_version=4; live_route_epoch=0; transport_epoch=0; reason=header_switch_off; proven_committed_chunk=0; proven_highest_observed_chunk=-1"),
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_RuntimeUnlockProbeDuringFallbackWithoutAuthority_FailsSeparationProof()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var lines = new[]
        {
            ...
            LogLine($"event=runtime_unlock_precommit_probe_started; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; transaction_generation=4; offer_generation=7; tuna_path_lease_generation=3; probe_id=runtime-unlock-precommit:4:7:abc; target_route=file_tuna_v4; target_protocol_version=4; target_transport=tuna; handoff_kind=normal_to_tuna_activation"),
            LogLine($"event=runtime_unlock_precommit_probe_failed; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; transaction_generation=4; offer_generation=7; tuna_path_lease_generation=3; probe_id=runtime-unlock-precommit:4:7:abc; reason=timeout"),
            LogLine($"event=runtime_unlock_precommit_probe_ignored; direction=inbound; transfer_id={transferId}; session_id=sess_redacted; transaction_generation=4; offer_generation=7; tuna_path_lease_generation=3; probe_id=runtime-unlock-precommit:4:7:abc; target_transport=tuna; received_transport=regular_nkn; reason=not_tuna_transport"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_RuntimeUnlockAfterFallbackAuthorityGrant_PassesSeparationProof()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var lines = new[]
        {
            ...
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=file_tuna_v4; protocol_version=4; runtime_profile=file_tuna_v4_fast; frame_family=v4; handoff_kind=normal_to_tuna_activation; bridge_recovery_policy=tuna_strict; liveness_terminal_policy=file_tuna_v4_fast; selection_reason=file_tuna_active; file_tuna_active=1; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=1"),
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=2"),
            LogLine($"event=filetransfer_fallback_leg_authority_started; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=2; transport_epoch=9; bridge_recovery_generation=1; checkpoint_request_id=v6-regular-nkn-state-refresh:42; authority_reason=post_tuna_fallback_state_refresh_failed"),
            LogLine($"event=filetransfer_fallback_leg_authority_checkpoint_accepted; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=2; transport_epoch=9; bridge_recovery_generation=1; checkpoint_request_id=v6-regular-nkn-state-refresh:42; proven_committed_chunk=128; proven_highest_observed_chunk=160; reason=receiver_state"),
            LogLine($"event=filetransfer_runtime_unlock_waiting_for_fallback_survival; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; reason=runtime_unlock_probe_missing; rejection_reason=runtime_unlock_probe_missing; leg_id=leg:6; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=2; transport_epoch=9; bridge_recovery_generation=1; checkpoint_request_id=v6-regular-nkn-state-refresh:42; state=checkpoint_pending; can_send_data=0"),
            LogLine($"event=filetransfer_runtime_unlock_activation_window_granted; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; reason=fallback_checkpoint_accepted; leg_id=leg:6; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=2; transport_epoch=9; bridge_recovery_generation=1; checkpoint_request_id=v6-regular-nkn-state-refresh:42; state=recovery_active; can_send_data=1"),
            LogLine($"event=runtime_unlock_precommit_probe_started; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; transaction_generation=4; offer_generation=7; tuna_path_lease_generation=3; probe_id=runtime-unlock-precommit:4:7:abc; target_route=file_tuna_v4; target_protocol_version=4; target_transport=tuna; handoff_kind=normal_to_tuna_activation"),
            LogLine($"event=runtime_unlock_precommit_probe_acked; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; transaction_generation=4; offer_generation=7; tuna_path_lease_generation=3; probe_id=runtime-unlock-precommit:4:7:abc; received_transport=tuna"),
    ...
    public async Task AnalyzeRetained_InboundOnlyFallbackRoute_DoesNotRequireSenderAuthorityProof()
    {
        ...
        var lines = new[]
        {
            ...
            LogLine($"event=filetransfer_clean_fallback_leg_frozen; direction=inbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:1; leg_generation=1; route=file_tuna_v4; protocol_version=4; live_route_epoch=0; transport_epoch=0; reason=sidecar_remote_closed; proven_committed_chunk=0; proven_highest_observed_chunk=-1"),
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_SupersededControlPlaneDeliveryFromOlderLiveEpoch_FailsControlPlaneIsolationWithoutRouteMismatch()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var lines = new[]
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=2"),
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=file_tuna_v4; protocol_version=4; runtime_profile=file_tuna_v4_fast; frame_family=v4; handoff_kind=normal_to_tuna_activation; bridge_recovery_policy=tuna_strict; liveness_terminal_policy=file_tuna_v4_fast; selection_reason=file_tuna_active; file_tuna_active=1; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=3"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=2; leg_generation=3; bridge_recovery_generation=1; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:1; reason=superseded_fallback_checkpoint_request; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        Assert.Equal("pass", route["route_consistency_verdict"]);
        Assert.Equal("0", route["route_mismatch_count"]);
        Assert.Equal("fail", route["control_plane_isolation_verdict"]);
        Assert.Equal("0", route["control_plane_metadata_missing_count"]);
        Assert.Equal("1", route["fallback_control_plane_non_current_leg_generation_send_count"]);

        var verdict = ReadArtifactReport(result.ArtifactDir, "filetransfer-operator-verdict.txt");
        Assert.Equal("fallback_control_plane_stale_generation_treadmill", verdict["recovery_failure_class"]);
    }

    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_ControlPlaneNonCheckpointMessagesWithoutCheckpointId_DoNotFailMetadata()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var lines = new[]
        {
            ...
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=v6-regular-nkn-state-refresh:42; reason=checkpoint_request; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=receiver_state; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=(none); reason=receiver_state_progress; frame_type=filetransfer.receiver_state.v6; control_queue=1; control_copy=0; bulk_copy=1; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=frontier_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=(none); reason=frontier_stall_repair_due; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=1; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_CurrentFallbackReceiverStateProofWithoutCheckpointExchange_PassesIsolation()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var lines = new[]
        {
            ...
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=file_tuna_v4; protocol_version=4; runtime_profile=file_tuna_v4_fast; frame_family=v4; handoff_kind=normal_to_tuna_activation; bridge_recovery_policy=tuna_strict; liveness_terminal_policy=file_tuna_v4_fast; selection_reason=file_tuna_active; file_tuna_active=1; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=1"),
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=2"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=receiver_state; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=2; leg_generation=6; bridge_recovery_generation=1; transport_epoch=3; checkpoint_request_id=(none); reason=chunk_batch_committed; frame_type=filetransfer.receiver_state.v6; control_queue=1; control_copy=1; bulk_copy=1; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=receiver_state; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=2; leg_generation=6; bridge_recovery_generation=1; transport_epoch=3; checkpoint_request_id=(none); reason=terminal_progress; frame_type=filetransfer.receiver_state.v6; control_queue=1; control_copy=1; bulk_copy=1; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        var route = ReadArtifactReport(result.ArtifactDir, "filetransfer-route-consistency-summary.txt");
        Assert.Equal("pass", route["fallback_leg_authority_proof_verdict"]);
        Assert.Equal("2", route["fallback_leg_authority_continuous_receiver_state_proof_count"]);
        Assert.Equal("pass", route["control_plane_isolation_verdict"]);
        Assert.Equal("0", route["control_plane_checkpoint_exchange_count"]);
        Assert.Equal("2", route["control_plane_continuous_receiver_state_proof_count"]);
        Assert.Equal("0", route["fallback_stale_generation_treadmill_score"]);
        Assert.Equal("0", route["fallback_control_plane_non_current_leg_generation_send_count"]);
    }

    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_CheckpointFreeReceiverStateWithoutCurrentRoute_DoesNotInventNonCurrentSend()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
    ...
    public async Task AnalyzeRetained_RecoveredControlPlaneDeliveryFailure_DoesNotFailIsolation()
    {
        ...
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=4"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=v6-regular-nkn-state-refresh:42; reason=checkpoint_request_timeout; frame_type=filetransfer.frontier_request.v6; control_queue=0; control_copy=0; bulk_copy=0; peer_visible_any=0; accepted_any=0; local_only_rejected=0; control_queue_error=TimeoutException; control_copy_error=OperationCanceledException; bulk_copy_error=OperationCanceledException"),
            LogLine($"event=filetransfer_fallback_checkpoint_exchange_retired; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; retired_leg_id=leg:5; retired_leg_generation=5; current_leg_generation=5; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; transport_epoch=9; retired_checkpoint_request_id=v6-regular-nkn-state-refresh:42; current_checkpoint_request_id=(none); reason=delivery_failure_retry"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=v6-regular-nkn-state-refresh:43; reason=checkpoint_request_retry; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_proof; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=v6-regular-nkn-state-refresh:43; reason=checkpoint_proof_retry; frame_type=filetransfer.receiver_state.v6; control_queue=1; control_copy=0; bulk_copy=1; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        var route = ReadArtifactReport(result.ArtifactDir, "filetransfer-route-consistency-summary.txt");
        Assert.Equal("pass", route["control_plane_isolation_verdict"]);
        Assert.Equal("1", route["control_plane_delivery_failure_count"]);
        Assert.Equal("1", route["control_plane_delivery_failure_recovered_count"]);
        Assert.Equal("0", route["control_plane_liveness_timeout_during_valid_exchange_count"]);
    }

    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_ControlPlaneCheckpointProofWithoutCheckpointId_FailsMetadata()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=4"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_proof; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=(none); reason=checkpoint_proof; frame_type=filetransfer.receiver_state.v6; control_queue=1; control_copy=0; bulk_copy=1; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        var route = ReadArtifactReport(result.ArtifactDir, "filetransfer-route-consistency-summary.txt");
        Assert.Equal("fail", route["control_plane_isolation_verdict"]);
        Assert.Equal("1", route["control_plane_metadata_missing_count"]);
        var routeText = File.ReadAllText(Path.Combine(result.ArtifactDir, "filetransfer-route-consistency-summary.txt"));
        Assert.Contains("control-plane metadata missing", routeText, StringComparison.Ordinal);
    }

    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_BoundedStaleFallbackControlPlaneCleanup_DoesNotFailIsolation()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var lines = new[]
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=6"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; leg_generation=6; bridge_recovery_generation=3; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:6; reason=current_checkpoint_request; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=filetransfer_fallback_checkpoint_exchange_retired; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; retired_leg_id=leg:5; retired_leg_generation=5; current_leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=5; transport_epoch=1; retired_checkpoint_request_id=v6-regular-nkn-state-refresh:5; current_checkpoint_request_id=v6-regular-nkn-state-refresh:6; reason=new_fallback_leg"),
            LogLine($"event=filetransfer_control_plane_send_retired_stale_generation; direction=outbound; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=5; leg_generation=5; current_leg_generation=6; bridge_recovery_generation=2; current_bridge_recovery_generation=3; transport_epoch=1; current_transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:5; current_checkpoint_request_id=v6-regular-nkn-state-refresh:6; reason=stale_leg_generation; frame_type=filetransfer.frontier_request.v6"),
            LogLine($"event=filetransfer_fallback_stale_proof_ignored; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:6; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:5; expected_checkpoint_request_id=v6-regular-nkn-state-refresh:6; reason=checkpoint_request_mismatch"),
            LogLine($"event=filetransfer_fallback_stale_proof_ignored; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:6; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:5; expected_checkpoint_request_id=v6-regular-nkn-state-refresh:6; reason=checkpoint_request_mismatch"),
            LogLine($"event=filetransfer_fallback_stale_proof_replay_suppressed; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:6; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:5; expected_checkpoint_request_id=v6-regular-nkn-state-refresh:6; reason=checkpoint_request_mismatch"),
            LogLine($"event=filetransfer_fallback_stale_proof_replay_suppressed; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:6; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:5; expected_checkpoint_request_id=v6-regular-nkn-state-refresh:6; reason=checkpoint_request_mismatch"),
            LogLine($"event=filetransfer_post_tuna_fallback_state_refresh_deferred_discarded; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; reason=checkpoint_request_mismatch; deferred_checkpoint_request_id=v6-regular-nkn-state-refresh:5; current_checkpoint_request_id=v6-regular-nkn-state-refresh:6; deferred_transport_epoch=1; current_transport_epoch=1"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        var route = ReadArtifactReport(result.ArtifactDir, "filetransfer-route-consistency-summary.txt");
        Assert.Equal("pass", route["control_plane_isolation_verdict"]);
        Assert.Equal("6", route["fallback_stale_generation_treadmill_score"]);
        Assert.Equal("6", route["fallback_stale_generation_treadmill_score_limit"]);
        Assert.Equal("1", route["control_plane_stale_send_retired_count"]);
        Assert.Equal("2", route["fallback_stale_proof_ignored_count"]);
        Assert.Equal("2", route["fallback_stale_proof_replay_suppressed_count"]);
        Assert.Equal("1", route["fallback_stale_deferred_replay_discarded_count"]);
        Assert.Equal("0", route["fallback_checkpoint_owner_violation_count"]);
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_StaleFallbackControlPlaneReplay_IsClassifiedAsTreadmill()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        for (var i = 0; i < 7; i++)
        {
            lines.Add(LogLine($"event=filetransfer_fallback_stale_proof_ignored; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:6; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:5; expected_checkpoint_request_id=(none); reason=checkpoint_not_pending"));
        }

        lines.Add(LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"));
        lines.Add(LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1"));

        var result = await RunAnalyzeFixtureAsync(lines);

        var route = ReadArtifactReport(result.ArtifactDir, "filetransfer-route-consistency-summary.txt");
        Assert.Equal("fail", route["control_plane_isolation_verdict"]);
        Assert.Equal("7", route["fallback_control_plane_stale_replay_treadmill_count"]);
        Assert.Equal("7", route["fallback_stale_generation_treadmill_score"]);
        Assert.Equal("6", route["fallback_stale_generation_treadmill_score_limit"]);
        Assert.Equal("7", route["fallback_stale_proof_ignored_count"]);
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_FallbackControlLaneStarvationUnderBulkReceive_IsClassified()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4; file_tuna_active=0; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=nkn"),
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=6", secondsOffset: 1),
            LogLine($"event=filetransfer_fallback_leg_authority_started; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_generation=6; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; transport_epoch=12; bridge_recovery_generation=3; checkpoint_request_id=v6-regular-nkn-state-refresh:77; authority_reason=post_tuna_fallback_state_refresh_failed", secondsOffset: 2),
            LogLine($"event=filetransfer_post_tuna_fallback_control_plane_pressure_marked; session_id=sess_redacted; transfer_id={transferId}; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; leg_generation=6; bridge_recovery_generation=3; transport_epoch=12; checkpoint_request_id=v6-regular-nkn-state-refresh:77; kind=receiverstate; reason=post_tuna_fallback_control_plane_pressure; ttl_ms=20000; active_file_transfer_data_sessions=1; active_file_transfer_data_bytes=268435456", secondsOffset: 3),
            LogLine($"event=nkn_bridge_control_receive_degraded; control_messages_received_since_last=0; bulk_messages_received_since_last=24; control_last_received_age_ms=770000; bulk_last_received_age_ms=11; regular_v4_control_feedback_pressure=0; post_tuna_fallback_control_plane_pressure=1; sample_window_ms=5000", secondsOffset: 20),
            LogLine($"event=nkn_bridge_control_receive_recovery_suppressed; reason=filetransfer_bulk_receive_active; control_messages_received_since_last=0; bulk_messages_received_since_last=24; control_last_received_age_ms=770000; bulk_last_received_age_ms=11; regular_v4_control_feedback_pressure=0; post_tuna_fallback_control_plane_pressure=1; pressure_grace_exhausted=1; protocol_repair_grace_windows=2; protocol_repair_grace_max_age_ms=15000; session_id=sess_redacted; transfer_id={transferId}; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=6; leg_generation=6; transport_epoch=12; bridge_recovery_generation=3; checkpoint_request_id=v6-regular-nkn-state-refresh:77; pressure_kind=receiverstate; pressure_reason=post_tuna_fallback_control_plane_pressure; pressure_recovery_attempted=0", secondsOffset: 21),
            LogLine("event=session_liveness_timeout; session_id=sess_redacted; generation=1; silence_ms=90000; terminal_timeout_ms=18000; role=Helper", secondsOffset: 90),
            LogLine($"event=file_transfer_outbound_terminal; role=Helpee; session_id=sess_redacted; transfer_id={transferId}; state=Failed; error_code=peer_disconnected", secondsOffset: 91),
            LogLine($"event=file_transfer_inbound_terminal; role=Helper; session_id=sess_redacted; transfer_id={transferId}; state=Failed; error_code=peer_disconnected; saved_path=(none)", secondsOffset: 92)
        };

        var result = await RunAnalyzeFixtureAsync(lines);

        var verdict = ReadArtifactReport(result.ArtifactDir, "filetransfer-operator-verdict.txt");
        Assert.Equal("1", verdict["post_tuna_fallback_control_plane_pressure_marked_count"]);
        Assert.Equal("1", verdict["post_tuna_fallback_bulk_active_suppressed_during_pressure_count"]);
        Assert.Equal("0", verdict["post_tuna_fallback_control_plane_pressure_allowed_recovery_count"]);
        Assert.Equal("fallback_control_lane_starvation_under_bulk_receive", verdict["recovery_failure_class"]);
    }

    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_SameFallbackLegUsingSecondCheckpointIdWithoutRetirement_FailsOwnerGuardrail()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var lines = new[]
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=4"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=v6-regular-nkn-state-refresh:42; reason=checkpoint_request; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=9; checkpoint_request_id=v6-regular-nkn-state-refresh:43; reason=checkpoint_request_replay; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        var route = ReadArtifactReport(result.ArtifactDir, "filetransfer-route-consistency-summary.txt");
        Assert.Equal("fail", route["control_plane_isolation_verdict"]);
        Assert.Equal("1", route["fallback_checkpoint_owner_violation_count"]);
        Assert.Equal("2", route["fallback_checkpoint_id_count_max_per_leg"]);
        Assert.Contains("5:v6-regular-nkn-state-refresh:42|v6-regular-nkn-state-refresh:43", route["fallback_checkpoint_ids_by_leg_generation"], StringComparison.Ordinal);
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_SameFallbackLegUsingSecondCheckpointIdAfterRetirement_PassesOwnerGuardrail()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; runtime_profile=default_v6; frame_family=v6; handoff_kind=tuna_to_normal_fallback; bridge_recovery_policy=post_tuna_fallback_strict; liveness_terminal_policy=post_tuna_fallback_v6_repair; selection_reason=post_tuna_file_fallback_active; file_tuna_active=0; post_tuna_fallback_active=1; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=4"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:42; reason=checkpoint_request; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=filetransfer_fallback_checkpoint_exchange_retired; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; retired_leg_id=leg:5; retired_leg_generation=5; current_leg_generation=5; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; transport_epoch=1; retired_checkpoint_request_id=v6-regular-nkn-state-refresh:42; current_checkpoint_request_id=(none); reason=transport_epoch_adopted"),
            LogLine($"event=filetransfer_fallback_leg_transport_epoch_adopted; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; leg_id=leg:5; leg_generation=5; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; previous_transport_epoch=1; transport_epoch=2; bridge_recovery_generation=2; retired_checkpoint_request_id=v6-regular-nkn-state-refresh:42; current_checkpoint_request_id=(none); state=checkpoint_pending; can_send_data=0; reason=receiver_state_sparse_runtime"),
            LogLine($"event=filetransfer_control_plane_delivery_result; kind=fallback_checkpoint_request; transfer_id={transferId}; session_id=sess_redacted; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; leg_generation=5; bridge_recovery_generation=2; transport_epoch=2; checkpoint_request_id=v6-regular-nkn-state-refresh:43; reason=checkpoint_request_after_transport_epoch_adopted; frame_type=filetransfer.frontier_request.v6; control_queue=1; control_copy=1; bulk_copy=0; peer_visible_any=1; accepted_any=1; local_only_rejected=0"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        Assert.Equal("0", route["fallback_checkpoint_owner_violation_count"]);
        Assert.Equal("2", route["fallback_checkpoint_id_count_max_per_leg"]);
        Assert.Equal("1", route["fallback_checkpoint_exchange_retired_count"]);
        Assert.Contains("5:v6-regular-nkn-state-refresh:42|v6-regular-nkn-state-refresh:43", route["fallback_checkpoint_ids_by_leg_generation"], StringComparison.Ordinal);
    ...
    public async Task AnalyzeRetained_FallbackTailReconciliationEvidence_IsReported()
    {
        ...
        var lines = new[]
        {
            ...
            LogLine($"event=filetransfer_fallback_checkpoint_exchange_retired; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; retired_leg_id=leg:5; retired_leg_generation=5; current_leg_generation=5; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=4; transport_epoch=9; retired_checkpoint_request_id=v6-regular-nkn-state-refresh:tail; current_checkpoint_request_id=(none); reason=tail_reconciliation_forced"),
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_DeferredNormalToTunaRouteHint_DoesNotCountAsRouteMismatch()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        {
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4; file_tuna_active=0; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=nkn"),
            LogLine($"event=filetransfer_normal_to_tuna_handoff_route_hint_deferred; transfer_id={transferId}; source=handoff_broadcast; reason=tuna_activation_negotiated; route=file_tuna_v4; protocol_version=4"),
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id=sess_redacted; route=file_tuna_v4; protocol_version=4; runtime_profile=file_tuna_v4_fast; frame_family=v4; handoff_kind=normal_to_tuna_activation; bridge_recovery_policy=tuna_strict; liveness_terminal_policy=file_tuna_v4_fast; selection_reason=file_tuna_active; file_tuna_active=1; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=nkn; live_route_epoch=1"),
            LogLine($"event=file_transfer_inbound_terminal; role=helper; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); saved_path=(none); integrity_ok=1"),
            LogLine($"event=file_transfer_outbound_terminal; role=helpee; session_id=sess_redacted; transfer_id={transferId}; state=Completed; error_code=(none); integrity_ok=1")
        };
        ...
        Assert.Equal("regular_nkn_v4_fast,file_tuna_v4", route["selected_route_changes"]);
        var routeText = File.ReadAllText(Path.Combine(result.ArtifactDir, "filetransfer-route-consistency-summary.txt"));
        Assert.DoesNotContain("filetransfer_normal_to_tuna_handoff_route_hint_deferred", routeText, StringComparison.Ordinal);
    }
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_PostTunaFallbackFrontierRepairCountAndRateOverCap_WithCleanFallbackProofReturnsWarning()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        lines.InsertRange(
            firstTerminalIndex,
            [
                LogLine("event=filetransfer_fallback_leg_authority_checkpoint_accepted; direction=outbound; transfer_id=[redacted]; session_id=sess_redacted; leg_id=leg:2; leg_generation=2; route=post_tuna_fallback_v6; protocol_version=6; live_route_epoch=1; transport_epoch=1; checkpoint_request_id=v6-regular-nkn-state-refresh:1; proven_committed_chunk=512; proven_highest_observed_chunk=511; state=recovery_active; reason=receiver_state_sparse_runtime", secondsOffset: 4),
            ]);
        var churnLines = new List<string>();
        for (var i = 0; i < 10; i++)
        {
        ...
        Assert.Equal("(none)", verdict["warning_cap_exceeded_kinds"]);
        Assert.Equal("fallback_frontier_repair_churn", verdict["warning_cap_exempted_kinds"]);
        Assert.Equal("fallback_frontier_repair_churn:4", verdict["warning_kind_counts"]);
        Assert.Equal("fallback_frontier_repair_churn:10", verdict["warning_kind_raw_event_counts"]);
        Assert.Equal("fallback_frontier_repair_churn:post_tuna_fallback", verdict["warning_cap_contexts"]);
        Assert.Equal("pass", verdict["fallback_leg_authority_proof_verdict"]);
        Assert.Equal("pass", verdict["bridge_liveness_integration_verdict"]);
    }
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_RegularV4RecoveryRewindTreadmill_IsClassified()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        const string transferId = "transfer_regular_v4_rewind_treadmill";
        var lines = StretchTransferWindowForWarningRate(BuildRouteAwareCompletedFixture(
            transferId,
            route: "regular_nkn_v4_fast",
            protocolVersion: 4,
        ...
        lines.InsertRange(
            firstTerminalIndex,
            [
                LogLine($"event=filetransfer_regular_v4_receive_recovery_frontier_rebind; direction=outbound; transfer_id={transferId}; session_id=sess_a; route=regular_nkn_v4_fast; protocol_version=4; remote_next_expected_chunk_index=1400; chunks_accepted_before=3900; frontier_lag_chunks=2500; replay_or_recovery_active=1", secondsOffset: 20),
                LogLine($"event=filetransfer_v4_frontier_stall_missing_range_due; direction=inbound; transfer_id={transferId}; session_id=sess_a; next_chunk_index=1400; highest_received_chunk_index=3900; missing_range_count=12; missing_chunk_count=512", secondsOffset: 21),
                LogLine($"event=filetransfer_v4_repair_requested; direction=inbound; transfer_id={transferId}; session_id=sess_a; request_key=frontier:1400-1911; missing_chunk_count=512; reason=frontier_stall_missing_range", secondsOffset: 22),
                LogLine($"event=filetransfer_sender_repair_cache_pressure_entered; direction=outbound; transfer_id={transferId}; session_id=sess_a; sent_cache_chunk_count=2048; sent_cache_bytes=44040192", secondsOffset: 23),
                LogLine($"event=filetransfer_sender_repair_chunk_skipped; transfer_id={transferId}; session_id=sess_a; reason=not_yet_sent; chunk_index=1400; remote_next_expected_chunk_index=1400; chunks_accepted_for_transport=1400; chunk_count=12483", secondsOffset: 24),
                LogLine($"event=filetransfer_regular_v4_receive_recovery_frontier_rebind; direction=outbound; transfer_id={transferId}; session_id=sess_a; route=regular_nkn_v4_fast; protocol_version=4; remote_next_expected_chunk_index=1400; chunks_accepted_before=2500; frontier_lag_chunks=1100; replay_or_recovery_active=1", secondsOffset: 26),
                LogLine($"event=filetransfer_regular_v4_receive_recovery_frontier_rebind; direction=outbound; transfer_id={transferId}; session_id=sess_a; route=regular_nkn_v4_fast; protocol_version=4; remote_next_expected_chunk_index=1400; chunks_accepted_before=2200; frontier_lag_chunks=800; replay_or_recovery_active=1", secondsOffset: 28),
                LogLine($"event=filetransfer_regular_v4_receive_recovery_frontier_rebind; direction=outbound; transfer_id={transferId}; session_id=sess_a; route=regular_nkn_v4_fast; protocol_version=4; remote_next_expected_chunk_index=1400; chunks_accepted_before=2100; frontier_lag_chunks=700; replay_or_recovery_active=1", secondsOffset: 30),
                LogLine($"event=filetransfer_v4_efficiency_summary; direction=outbound; transfer_id={transferId}; session_id=sess_a; terminal_state=completed; route=regular_nkn_v4_fast; protocol_version=4; file_size_bytes=268435456; bytes_acknowledged_by_receiver=268435456; bytes_transferred=268435456; chunks_accepted_for_transport=12483; remote_frontier_chunk_index=12483; raw_bytes_sent_total=1500000000; normal_raw_bytes_sent_total=700000000; repair_raw_bytes_sent_total=800000000; raw_to_file_permille=5588; normal_to_file_permille=2607; repair_to_file_permille=2980; repair_to_raw_permille=533; send_wait_count_total=42", secondsOffset: 36)
            ]);
        ...
        var verdict = ReadArtifactReport(result.ArtifactDir, "filetransfer-operator-verdict.txt");
        Assert.Equal("(none)", verdict["recovery_failure_class"]);
        Assert.Equal("regular_v4_recovery_rewind_treadmill", verdict["regular_v4_efficiency_pressure_class"]);
        Assert.Equal("4", verdict["regular_v4_frontier_rebind_count"]);
        Assert.Equal("0", verdict["regular_v4_frontier_rebind_rejected_count"]);
        Assert.Equal("1", verdict["regular_v4_frontier_stall_missing_range_count"]);
        Assert.Equal("1", verdict["regular_v4_repair_requested_count"]);
        Assert.Equal("1", verdict["regular_v4_repair_cache_pressure_count"]);

        var stability = ReadArtifactReport(result.ArtifactDir, "stability-gates-summary.txt");
        Assert.Equal("regular_v4_recovery_rewind_treadmill", stability["regular_v4_efficiency_pressure_class"]);
    }
    ...
    public async Task AnalyzeRetained_RuntimeUnlockDispatchDeferredByRegularV4ReceiveRecovery_ClassifiesNextBlocker()
    {
        ...
        var verdict = ReadArtifactReport(result.ArtifactDir, "filetransfer-operator-verdict.txt");
        Assert.Equal("runtime_unlock_regular_v4_recovery_arbitration", verdict["recovery_failure_class"]);
        Assert.Equal("1", verdict["runtime_unlock_offer_not_observed_count"]);
        Assert.Equal("1", verdict["runtime_unlock_retry_dispatched_count"]);
        Assert.Equal("1", verdict["runtime_unlock_dispatch_deferred_for_regular_v4_recovery_count"]);
        Assert.Equal("0", verdict["runtime_unlock_regular_v4_activation_window_started_count"]);
        ...
        var stability = ReadArtifactReport(result.ArtifactDir, "stability-gates-summary.txt");
        Assert.Equal("runtime_unlock_regular_v4_recovery_arbitration", stability["recovery_failure_class"]);
        Assert.Equal("1", stability["runtime_unlock_dispatch_deferred_for_regular_v4_recovery_count"]);
        Assert.Equal("fail", stability["runtime_unlock_regular_v4_recovery_arbitration_verdict"]);
    }

    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_RuntimeUnlockRegularV4ActivationWindowPeerVisible_PassesArbitrationProof()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var result = await RunAnalyzeFixtureAsync(
            BuildRuntimeUnlockRegularV4ActivationWindowPeerVisibleFixture(),
            ["-LiveRouteProofMode", "None"]);
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_RuntimeUnlockTransactionLocalOnlyObservedSend_ClassifiesTransactionFailure()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var result = await RunAnalyzeFixtureAsync(
            BuildRuntimeUnlockTransactionLocalOnlyObservedSendFixture(),
            ["-LiveRouteProofMode", "RegularActivationCycle"]);

        var verdict = ReadArtifactReport(result.ArtifactDir, "filetransfer-operator-verdict.txt");
        Assert.Equal("runtime_unlock_transaction_local_only", verdict["recovery_failure_class"]);
        ...
        Assert.Equal("1", verdict["runtime_unlock_transaction_local_only_rejected_count"]);
        Assert.Equal("fail", verdict["runtime_unlock_transaction_proof_verdict"]);

        var stability = ReadArtifactReport(result.ArtifactDir, "stability-gates-summary.txt");
        Assert.Equal("runtime_unlock_transaction_local_only", stability["recovery_failure_class"]);
        Assert.Equal("fail", stability["runtime_unlock_transaction_proof_verdict"]);
    }

    [Fact]
    [Trait("Category", "Smoke")]
    public async Task AnalyzeRetained_RuntimeUnlockTransactionPeerProofAndRouteCommit_IsAccepted()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var result = await RunAnalyzeFixtureAsync(
            BuildRuntimeUnlockTransactionPeerProofAndRouteCommitFixture(),
            ["-LiveRouteProofMode", "RegularActivationCycle"]);
    ...
    public async Task AnalyzeRetained_RuntimeUnlockTunaPathLeaseUnavailableAtAnswer_ClassifiesLeaseFailure()
    {
        ...
        var result = await RunAnalyzeFixtureAsync(
            BuildRuntimeUnlockTunaPathLeaseUnavailableAtAnswerFixture(),
            ["-LiveRouteProofMode", "RegularActivationCycle"]);

        var verdict = ReadArtifactReport(result.ArtifactDir, "filetransfer-operator-verdict.txt");
        Assert.Equal("tuna_path_lease_unavailable_at_answer", verdict["recovery_failure_class"]);
        Assert.Equal("1", verdict["runtime_unlock_tuna_path_lease_started_count"]);
        Assert.Equal("1", verdict["runtime_unlock_tuna_path_lease_ready_count"]);
        Assert.Equal("1", verdict["runtime_unlock_tuna_path_lease_failed_count"]);
        Assert.Equal("1", verdict["runtime_unlock_answer_rejected_tuna_path_lease_unavailable_count"]);
        Assert.Equal("fail", verdict["runtime_unlock_transaction_proof_verdict"]);

        var stability = ReadArtifactReport(result.ArtifactDir, "stability-gates-summary.txt");
        Assert.Equal("tuna_path_lease_unavailable_at_answer", stability["recovery_failure_class"]);
        Assert.Equal("1", stability["runtime_unlock_answer_rejected_tuna_path_lease_unavailable_count"]);
    }
...
""",
    ...
    [Fact]
    [Trait("Category", "Smoke")]
    public async Task RunFileTransferRouteAcceptance_Phase5SecondTransferReadinessUnstableRerunsCleanAttempt()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var repoRoot = FindRepoRoot();
        var artifactRoot = Path.Combine(repoRoot, "artifacts", "filetransfer-route-acceptance-test", Guid.NewGuid().ToString("N"));
        var runRoot = Path.Combine(artifactRoot, "phase5-second-transfer-readiness-unstable-rerun-pass");

        try
        {
            var environment = BuildFakeRouteAcceptanceEnvironment("phase5-second-transfer-readiness-unstable-rerun-pass");
            environment["NLINK_FILETRANSFER_ROUTE_ACCEPTANCE_FAKE_PHASE4_SECOND_TRANSFER_AFTER_REACTIVATION_TRANSIENT_SETUP_FAILURE"] = "1";
            environment["NLINK_FILETRANSFER_ROUTE_ACCEPTANCE_FAKE_PHASE4_SECOND_TRANSFER_AFTER_REACTIVATION_TRANSIENT_SETUP_PHASE"] = "preactivation_readiness";
            environment["NLINK_FILETRANSFER_ROUTE_ACCEPTANCE_FAKE_PHASE4_SECOND_TRANSFER_AFTER_REACTIVATION_TRANSIENT_SETUP_REASON"] = "second_transfer_tuna_readiness_unstable";
            ...
            Assert.True(
                result.ExitCode == 0,
                $"Expected second-transfer readiness instability to require and pass a clean Phase 5 rerun.{Environment.NewLine}STDOUT:{Environment.NewLine}{result.Stdout}{Environment.NewLine}STDERR:{Environment.NewLine}{result.Stderr}");
            var summary = ReadArtifactReport(runRoot, "phase5-analyzer-gui-acceptance-summary.txt");
            Assert.Equal("PASS", summary["verdict"]);
            Assert.Equal("1", summary["second-transfer-after-reactivation.retry_used"]);
            Assert.Equal("2", summary["second-transfer-after-reactivation.selected_attempt"]);
            Assert.False(summary.ContainsKey("second-transfer-after-reactivation.setup_failure_phase"));
            Assert.False(summary.ContainsKey("second-transfer-after-reactivation.setup_failure_reason"));
            Assert.Contains("second_transfer_tuna_readiness_unstable", summary["second-transfer-after-reactivation.first_failure_reason"], StringComparison.Ordinal);
            Assert.Equal("none", summary["second-transfer-after-reactivation.acceptance_failure_class"]);
            Assert.True(Directory.Exists(Path.Combine(runRoot, "second-transfer-after-reactivation-rerun-1")));
        }
        finally
        {
            TryDeleteDirectory(artifactRoot);
    ...
    [InlineData("NLINK_FILETRANSFER_ROUTE_ACCEPTANCE_FAKE_PHASE4_SECOND_TRANSFER_AFTER_REACTIVATION_SECOND_TRANSFER_NOT_STARTED", "1", "setup", "second transfer setup failure after reactivation")]
    ...
    private static string[] BuildRuntimeUnlockRegularV4ActivationWindowPeerVisibleFixture()
    {
        const string transferId = "transfer_runtime_unlock_regular_v4_window_peer_visible";
        const string sessionId = "sess_runtime_unlock_regular_v4_window_peer_visible";
        return
        [
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id={sessionId}; route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4; file_tuna_active=0; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=conservative_nkn_startup", secondsOffset: 0),
    ...
    private static string[] BuildRuntimeUnlockTransactionLocalOnlyObservedSendFixture()
    {
        const string transferId = "transfer_runtime_unlock_transaction_local_only";
        const string sessionId = "sess_runtime_unlock_transaction_local_only";
        return
        [
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id={sessionId}; route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4; file_tuna_active=0; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=conservative_nkn_startup", secondsOffset: 0),
            LogLine($"event=filetransfer_route_selected; direction=inbound; transfer_id={transferId}; session_id={sessionId}; route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4; file_tuna_active=0; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=conservative_nkn_startup", secondsOffset: 1),
            LogLine($"event=runtime_unlock_transaction_offer_generation_created; session_id={sessionId}; transaction_generation=1; offer_generation=7; state=offerpreparing; peer_visible_proof=0; peer_received=0; answer_received=0; route_commit_pending=0; route_committed=0; failure_reason=(none); retired_reason=(none); reason=runtime_unlock", secondsOffset: 10),
            LogLine($"event=runtime_unlock_transaction_observed_send; session_id={sessionId}; transaction_generation=1; offer_generation=7; state=offersentobserved; peer_visible_proof=0; peer_received=0; answer_received=0; route_commit_pending=0; route_committed=0; failure_reason=(none); retired_reason=(none); reason=offer_observed_send", secondsOffset: 11),
            LogLine($"event=filetransfer_runtime_unlock_route_commit_rejected; direction=outbound; transfer_id={transferId}; session_id={sessionId}; reason=normal_to_tuna_activation; rejection_reason=transaction_proof_missing", secondsOffset: 12),
            LogLine($"event=session_liveness_timeout; session_id={sessionId}; generation=1; silence_ms=90000; terminal_timeout_ms=18000; role=Helper", secondsOffset: 90),
            LogLine($"event=file_transfer_outbound_terminal; role=Helpee; session_id={sessionId}; transfer_id={transferId}; state=Failed; error_code=peer_disconnected", secondsOffset: 91),
    ...
    private static string[] BuildRuntimeUnlockTransactionPeerProofAndRouteCommitFixture()
    {
        const string transferId = "[redacted]";
        const string sessionId = "sess_redacted";
        return
        [
    ...
    private static string[] BuildRuntimeUnlockTransactionPendingCommitAfterPriorCommitFixture()
    {
        ...
        [
            ...
            LogLine($"event=runtime_unlock_transaction_offer_generation_created; session_id={sessionId}; transaction_generation=2; offer_generation=9; state=offerpreparing; peer_visible_proof=0; peer_received=0; answer_received=0; route_commit_pending=0; route_committed=0; failure_reason=(none); retired_reason=(none); reason=peer_runtime_unlock_offer_accepted", secondsOffset: 20),
            LogLine($"event=runtime_unlock_transaction_peer_received; session_id={sessionId}; transaction_generation=2; offer_generation=9; state=peerreceived; peer_visible_proof=1; peer_received=1; answer_received=0; route_commit_pending=1; route_committed=0; failure_reason=(none); retired_reason=(none); reason=transport_acceleration_offer_received", secondsOffset: 21),
    ...
    private static string[] BuildRuntimeUnlockTunaPathLeaseUnavailableAtAnswerFixture()
    {
        const string transferId = "transfer_runtime_unlock_tuna_path_lease_unavailable";
        const string sessionId = "sess_runtime_unlock_tuna_path_lease_unavailable";
        return
        [
            LogLine($"event=filetransfer_route_selected; direction=outbound; transfer_id={transferId}; session_id={sessionId}; route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4; file_tuna_active=0; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=conservative_nkn_startup", secondsOffset: 0),
            LogLine($"event=filetransfer_route_selected; direction=inbound; transfer_id={transferId}; session_id={sessionId}; route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4; file_tuna_active=0; post_tuna_fallback_active=0; diagnostic_regular_nkn_v6=0; transport_profile=conservative_nkn_startup", secondsOffset: 1),
            LogLine($"event=tuna_path_lease_started; session_id={sessionId}; transfer_id=(none); transaction_generation=0; offer_generation=0; lease_generation=3; listener_run_id=listener-start-3; payer_decision_id=5; state=listenerstarting; reason=runtime_unlock", secondsOffset: 2),
            LogLine($"event=tuna_path_lease_listener_ready; session_id={sessionId}; transfer_id=(none); transaction_generation=0; offer_generation=0; lease_generation=3; listener_run_id=listener-start-3; payer_decision_id=5; state=listenerready; current=1; failure_reason=(none); retired_reason=(none); reason=runtime_unlock", secondsOffset: 3),
            LogLine($"event=runtime_unlock_transaction_offer_generation_created; session_id={sessionId}; transaction_generation=1; offer_generation=7; state=offerpreparing; peer_visible_proof=0; peer_received=0; answer_received=0; route_commit_pending=0; route_committed=0; failure_reason=(none); retired_reason=(none); tuna_path_lease_generation=3; tuna_path_lease_state=listenerready; tuna_path_lease_current=1; tuna_path_lease_listener_run_id=listener-start-3; tuna_path_lease_failure_reason=(none); reason=runtime_unlock", secondsOffset: 4),
            LogLine($"event=runtime_unlock_transaction_observed_send; session_id={sessionId}; transaction_generation=1; offer_generation=7; state=offersentobserved; peer_visible_proof=0; peer_received=0; answer_received=0; route_commit_pending=0; route_committed=0; failure_reason=(none); retired_reason=(none); tuna_path_lease_generation=3; tuna_path_lease_state=listenerready; tuna_path_lease_current=1; tuna_path_lease_listener_run_id=listener-start-3; tuna_path_lease_failure_reason=(none); reason=offer_observed_send", secondsOffset: 5),
            LogLine($"event=runtime_unlock_transaction_peer_received; session_id={sessionId}; transaction_generation=1; offer_generation=7; state=peerreceived; peer_visible_proof=1; peer_received=1; answer_received=0; route_commit_pending=1; route_committed=0; failure_reason=(none); retired_reason=(none); tuna_path_lease_generation=3; tuna_path_lease_state=listenerready; tuna_path_lease_current=1; tuna_path_lease_listener_run_id=listener-start-3; tuna_path_lease_failure_reason=(none); reason=transport_acceleration_answer", secondsOffset: 6),
            LogLine($"event=runtime_unlock_answer_rejected_tuna_path_lease_unavailable; session_id={sessionId}; payer_decision_id=5; offer_generation=7; reject_reason=sidecar_unavailable", secondsOffset: 7),
            LogLine($"event=tuna_path_lease_failed; session_id={sessionId}; transfer_id=(none); transaction_generation=1; offer_generation=7; lease_generation=3; listener_run_id=listener-start-3; payer_decision_id=5; state=failed; current=0; failure_reason=runtime_unlock_answer_rejected_tuna_path_lease_unavailable; retired_reason=(none); reason=runtime_unlock_answer_rejected_tuna_path_lease_unavailable", secondsOffset: 8),
            LogLine($"event=runtime_unlock_transaction_failed; session_id={sessionId}; transaction_generation=1; offer_generation=7; state=failed; peer_visible_proof=1; peer_received=1; answer_received=0; route_commit_pending=0; route_committed=0; failure_reason=runtime_unlock_answer_rejected_tuna_path_lease_unavailable; retired_reason=(none); tuna_path_lease_generation=3; tuna_path_lease_state=failed; tuna_path_lease_current=0; tuna_path_lease_listener_run_id=listener-start-3; tuna_path_lease_failure_reason=runtime_unlock_answer_rejected_tuna_path_lease_unavailable; reason=runtime_unlock_answer_rejected_tuna_path_lease_unavailable", secondsOffset: 9),
            LogLine($"event=filetransfer_runtime_unlock_route_commit_rejected; direction=outbound; transfer_id={transferId}; session_id={sessionId}; reason=normal_to_tuna_activation; rejection_reason=tuna_path_lease_failed", secondsOffset: 10),
            LogLine($"event=session_liveness_timeout; session_id={sessionId}; generation=1; silence_ms=90000; terminal_timeout_ms=18000; role=Helper", secondsOffset: 90),
            LogLine($"event=file_transfer_outbound_terminal; role=Helpee; session_id={sessionId}; transfer_id={transferId}; state=Failed; error_code=peer_disconnected", secondsOffset: 91),
            LogLine($"event=transfer_terminal; direction=outbound; transfer_id={transferId}; session_id={sessionId}; file_name_len=33; file_size_bytes=268435456; bytes_transferred=0; chunks_transferred=0; chunk_count=12481; error_code=peer_disconnected; reason=Peer disconnected.; saved_path=(none); route=regular_nkn_v4_fast; protocol_version=4; runtime_profile=regular_nkn_v4_fast; frame_family=v4; handoff_kind=none; bridge_recovery_policy=regular_nkn_v4_fast; liveness_terminal_policy=regular_nkn_v4_fast; selection_reason=regular_nkn_default_v4", secondsOffset: 92)
        ];
