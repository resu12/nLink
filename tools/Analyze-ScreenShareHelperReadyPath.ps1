...
function Get-LastReadyPathSummaryLine {
    param([Parameter(Mandatory = $true)][string]$Path)
    ...
    foreach ($line in [System.IO.File]::ReadAllLines($Path)) {
        if (-not $inSummaryLinesSection) {
            if ([string]::Equals($line.Trim(), 'helper_ready_path_summary_lines:', [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        if ($line.IndexOf('screenshare_helper_ready_path_summary', [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
...
function Merge-ReadyPathValuesFromSummaryLine {
    param(
        [hashtable]$Values,
        [string]$SummaryPath
    )
    ...
    $summaryLine = Get-LastReadyPathSummaryLine -Path $SummaryPath
    ...
    $fallbackDefinitions = @(
        @{ Key = 'capture_to_first_fragment_observed_avg_ms'; Offset = 1 },
        @{ Key = 'capture_to_first_fragment_observed_median_ms'; Offset = 2 },
        @{ Key = 'capture_to_first_fragment_observed_p95_ms'; Offset = 3 },
        @{ Key = 'capture_to_first_fragment_observed_max_ms'; Offset = 4 },
        @{ Key = 'first_fragment_to_last_fragment_observed_avg_ms'; Offset = 5 },
        @{ Key = 'first_fragment_to_last_fragment_observed_median_ms'; Offset = 6 },
        @{ Key = 'first_fragment_to_last_fragment_observed_p95_ms'; Offset = 7 },
        @{ Key = 'first_fragment_to_last_fragment_observed_max_ms'; Offset = 8 },
        @{ Key = 'last_fragment_to_assembly_complete_avg_ms'; Offset = 9 },
        @{ Key = 'last_fragment_to_assembly_complete_median_ms'; Offset = 10 },
        @{ Key = 'last_fragment_to_assembly_complete_p95_ms'; Offset = 11 },
        @{ Key = 'last_fragment_to_assembly_complete_max_ms'; Offset = 12 },
        @{ Key = 'assembly_complete_to_frame_emitted_avg_ms'; Offset = 13 },
        @{ Key = 'assembly_complete_to_frame_emitted_median_ms'; Offset = 14 },
        @{ Key = 'assembly_complete_to_frame_emitted_p95_ms'; Offset = 15 },
        @{ Key = 'assembly_complete_to_frame_emitted_max_ms'; Offset = 16 }
    )
    ...
    foreach ($stringKey in @('helper_session_phase', 'helper_recovery_mechanism', 'dominant_ready_path_stage')) {
...
function Get-FixAreaForClassification {
    ...
    switch -Exact ($Classification) {
        'receive_arrival_latency' { return 'media transport / receive-latency path' }
        'fragment_completion_latency' { return 'media transport / receive-latency path' }
        'assembly_latency' { return 'reassembler hot path' }
        'ready_emit_latency' { return 'reassembler hot path' }
        default { return 'mixed_or_inconclusive' }
    }
}

$stageOrder = @(
    @{ Name = 'capture_to_first_fragment_observed'; Classification = 'receive_arrival_latency' },
    @{ Name = 'first_fragment_to_last_fragment_observed'; Classification = 'fragment_completion_latency' },
    @{ Name = 'last_fragment_to_assembly_complete'; Classification = 'assembly_latency' },
    @{ Name = 'assembly_complete_to_frame_emitted'; Classification = 'ready_emit_latency' }
)
...
$candidateSummaryPath = Join-Path $CandidateArtifactDir 'helper-ready-path-summary.txt'
$candidateValues = Read-KeyValueSummaryFile -Path $candidateSummaryPath
$candidateValues = Merge-ReadyPathValuesFromSummaryLine -Values $candidateValues -SummaryPath $candidateSummaryPath
...
foreach ($referenceArtifactDir in $normalizedReferenceArtifactDirs) {
    ...
    $referenceSummaryPath = Join-Path $referenceArtifactDir 'helper-ready-path-summary.txt'
    if (-not (Test-Path $referenceSummaryPath)) {
        continue
    }

    $referenceValues = Read-KeyValueSummaryFile -Path $referenceSummaryPath
    $referenceValues = Merge-ReadyPathValuesFromSummaryLine -Values $referenceValues -SummaryPath $referenceSummaryPath
    $referenceSnapshots += @{
...
$reportLines.Add("reference_ready_path_summary_availability=$referenceSummaryAvailabilityCount/$($normalizedReferenceArtifactDirs.Count)")
$reportLines.Add("reference_ready_path_comparison_mode=$(if ($referenceComparisonAvailable) { 'reference_median_vs_max' } else { 'candidate_stage_composition_fallback' })")

foreach ($stageName in @(
    'capture_to_first_fragment_observed',
    'first_fragment_to_last_fragment_observed',
    'last_fragment_to_assembly_complete',
    'assembly_complete_to_frame_emitted')) {
...
$reportPath = Join-Path $CandidateArtifactDir 'helper-ready-path-analysis.txt'
