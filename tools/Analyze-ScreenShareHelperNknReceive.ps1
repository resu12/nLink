...
function Get-LastNknReceiveSummaryLine {
    param([Parameter(Mandatory = $true)][string]$Path)
    ...
    foreach ($line in [System.IO.File]::ReadAllLines($Path)) {
        if (-not $inSummaryLinesSection) {
            if ([string]::Equals($line.Trim(), 'helper_nkn_receive_summary_lines:', [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        if ($line.IndexOf('screenshare_helper_nkn_receive_summary', [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
...
function Merge-NknReceiveValuesFromSummaryLine {
    param(
        [hashtable]$Values,
        [string]$SummaryPath
    )
    ...
    $summaryLine = Get-LastNknReceiveSummaryLine -Path $SummaryPath
    ...
    $fallbackDefinitions = @(
        @{ Key = 'envelope_send_to_sdk_handle_msg_entered_avg_ms'; Offset = 1 },
        @{ Key = 'envelope_send_to_sdk_handle_msg_entered_median_ms'; Offset = 2 },
        @{ Key = 'envelope_send_to_sdk_handle_msg_entered_p95_ms'; Offset = 3 },
        @{ Key = 'envelope_send_to_sdk_handle_msg_entered_max_ms'; Offset = 4 },
        @{ Key = 'sdk_handle_msg_entered_to_client_message_dispatch_avg_ms'; Offset = 5 },
        @{ Key = 'sdk_handle_msg_entered_to_client_message_dispatch_median_ms'; Offset = 6 },
        @{ Key = 'sdk_handle_msg_entered_to_client_message_dispatch_p95_ms'; Offset = 7 },
        @{ Key = 'sdk_handle_msg_entered_to_client_message_dispatch_max_ms'; Offset = 8 },
        @{ Key = 'client_message_dispatch_to_multiclient_message_dispatch_avg_ms'; Offset = 9 },
        @{ Key = 'client_message_dispatch_to_multiclient_message_dispatch_median_ms'; Offset = 10 },
        @{ Key = 'client_message_dispatch_to_multiclient_message_dispatch_p95_ms'; Offset = 11 },
        @{ Key = 'client_message_dispatch_to_multiclient_message_dispatch_max_ms'; Offset = 12 },
        @{ Key = 'multiclient_message_dispatch_to_bridge_message_observed_avg_ms'; Offset = 13 },
        @{ Key = 'multiclient_message_dispatch_to_bridge_message_observed_median_ms'; Offset = 14 },
        @{ Key = 'multiclient_message_dispatch_to_bridge_message_observed_p95_ms'; Offset = 15 },
        @{ Key = 'multiclient_message_dispatch_to_bridge_message_observed_max_ms'; Offset = 16 }
    )
    ...
    foreach ($stringKey in @('helper_session_phase', 'helper_recovery_mechanism', 'dominant_nkn_receive_stage')) {
...
function Get-FixAreaForClassification {
    ...
    switch -Exact ($Classification) {
        'pre_sdk_handle_latency' { return 'NKN socket / JS event-loop receive work' }
        'sdk_client_inbound_processing_latency' { return 'nkn-sdk Client inbound handling' }
        'sdk_multiclient_fanin_latency' { return 'nkn-sdk MultiClient fan-in path' }
        'bridge_listener_entry_latency' { return 'bridge listener / instrumentation validation' }
        default { return 'mixed_or_inconclusive' }
    }
}

$stageOrder = @(
    @{ Name = 'envelope_send_to_sdk_handle_msg_entered'; Classification = 'pre_sdk_handle_latency' },
    @{ Name = 'sdk_handle_msg_entered_to_client_message_dispatch'; Classification = 'sdk_client_inbound_processing_latency' },
    @{ Name = 'client_message_dispatch_to_multiclient_message_dispatch'; Classification = 'sdk_multiclient_fanin_latency' },
    @{ Name = 'multiclient_message_dispatch_to_bridge_message_observed'; Classification = 'bridge_listener_entry_latency' }
)
...
$candidateSummaryPath = Join-Path $CandidateArtifactDir 'helper-nkn-receive-summary.txt'
$candidateValues = Read-KeyValueSummaryFile -Path $candidateSummaryPath
$candidateValues = Merge-NknReceiveValuesFromSummaryLine -Values $candidateValues -SummaryPath $candidateSummaryPath
...
foreach ($referenceArtifactDir in $normalizedReferenceArtifactDirs) {
    ...
    $referenceSummaryPath = Join-Path $referenceArtifactDir 'helper-nkn-receive-summary.txt'
    if (-not (Test-Path $referenceSummaryPath)) {
        continue
    }

    $referenceValues = Read-KeyValueSummaryFile -Path $referenceSummaryPath
    $referenceValues = Merge-NknReceiveValuesFromSummaryLine -Values $referenceValues -SummaryPath $referenceSummaryPath
    $referenceSnapshots += @{
...
$reportLines.Add("reference_nkn_receive_summary_availability=$referenceSummaryAvailabilityCount/$($normalizedReferenceArtifactDirs.Count)")
$reportLines.Add("reference_nkn_receive_comparison_mode=$referenceComparisonMode")
...
$reportPath = Join-Path $CandidateArtifactDir 'helper-nkn-receive-analysis.txt'
