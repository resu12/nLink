...
function Get-LastBridgeIngressSummaryLine {
    param([Parameter(Mandatory = $true)][string]$Path)
    ...
    foreach ($line in [System.IO.File]::ReadAllLines($Path)) {
        if (-not $inSummaryLinesSection) {
            if ([string]::Equals($line.Trim(), 'helper_bridge_ingress_summary_lines:', [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        if ($line.IndexOf('screenshare_helper_bridge_ingress_summary', [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
...
function Merge-BridgeIngressValuesFromSummaryLine {
    param(
        [hashtable]$Values,
        [string]$SummaryPath
    )
    ...
    $summaryLine = Get-LastBridgeIngressSummaryLine -Path $SummaryPath
    ...
    $fallbackDefinitions = @(
        @{ Key = 'envelope_send_to_bridge_message_observed_avg_ms'; Offset = 1 },
        @{ Key = 'envelope_send_to_bridge_message_observed_median_ms'; Offset = 2 },
        @{ Key = 'envelope_send_to_bridge_message_observed_p95_ms'; Offset = 3 },
        @{ Key = 'envelope_send_to_bridge_message_observed_max_ms'; Offset = 4 },
        @{ Key = 'bridge_message_observed_to_binary_frame_decoded_avg_ms'; Offset = 5 },
        @{ Key = 'bridge_message_observed_to_binary_frame_decoded_median_ms'; Offset = 6 },
        @{ Key = 'bridge_message_observed_to_binary_frame_decoded_p95_ms'; Offset = 7 },
        @{ Key = 'bridge_message_observed_to_binary_frame_decoded_max_ms'; Offset = 8 },
        @{ Key = 'binary_frame_decoded_to_bridge_ingress_avg_ms'; Offset = 9 },
        @{ Key = 'binary_frame_decoded_to_bridge_ingress_median_ms'; Offset = 10 },
        @{ Key = 'binary_frame_decoded_to_bridge_ingress_p95_ms'; Offset = 11 },
        @{ Key = 'binary_frame_decoded_to_bridge_ingress_max_ms'; Offset = 12 }
    )
    ...
    foreach ($stringKey in @('helper_session_phase', 'helper_recovery_mechanism', 'dominant_bridge_ingress_stage')) {
...
function Get-FixAreaForClassification {
    ...
    switch -Exact ($Classification) {
        'upstream_to_bridge_latency' { return 'JS bridge / NKN receive-side work' }
        'local_reader_backlog_latency' { return 'bridge mixed-stream reader / callback backlog' }
        'local_media_dispatch_latency' { return 'adapter-side media dispatch decoupling queue' }
        default { return 'mixed_or_inconclusive' }
    }
}

$stageOrder = @(
    @{ Name = 'envelope_send_to_bridge_message_observed'; Classification = 'upstream_to_bridge_latency' },
    @{ Name = 'bridge_message_observed_to_binary_frame_decoded'; Classification = 'local_reader_backlog_latency' },
    @{ Name = 'binary_frame_decoded_to_bridge_ingress'; Classification = 'local_media_dispatch_latency' }
)
...
$candidateSummaryPath = Join-Path $CandidateArtifactDir 'helper-bridge-ingress-summary.txt'
$candidateValues = Read-KeyValueSummaryFile -Path $candidateSummaryPath
$candidateValues = Merge-BridgeIngressValuesFromSummaryLine -Values $candidateValues -SummaryPath $candidateSummaryPath
...
foreach ($referenceArtifactDir in $normalizedReferenceArtifactDirs) {
    ...
    $referenceSummaryPath = Join-Path $referenceArtifactDir 'helper-bridge-ingress-summary.txt'
    if (-not (Test-Path $referenceSummaryPath)) {
        continue
    }

    $referenceValues = Read-KeyValueSummaryFile -Path $referenceSummaryPath
    $referenceValues = Merge-BridgeIngressValuesFromSummaryLine -Values $referenceValues -SummaryPath $referenceSummaryPath
    $referenceSnapshots += @{
...
$reportLines.Add("reference_bridge_ingress_summary_availability=$referenceSummaryAvailabilityCount/$($normalizedReferenceArtifactDirs.Count)")
$reportLines.Add("reference_bridge_ingress_comparison_mode=$(if ($referenceComparisonAvailable) { 'reference_median_vs_max' } else { 'candidate_stage_composition_fallback' })")

foreach ($stageName in @(
    'envelope_send_to_bridge_message_observed',
    'bridge_message_observed_to_binary_frame_decoded',
    'binary_frame_decoded_to_bridge_ingress')) {
...
$reportPath = Join-Path $CandidateArtifactDir 'helper-bridge-ingress-analysis.txt'
