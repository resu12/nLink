...
internal sealed class RealNknClientAdapter : INknClient, IBridgeProcessRunner, IAuthoritativeConnectedAddressSource, IBridgeScreenShareQueueCapability
{
    ...
    private static readonly TimeSpan RegularV4ActivationSendPressureTtl = TimeSpan.FromSeconds(30);
    private static readonly TimeSpan PostTunaFallbackControlPlanePressureTtl = TimeSpan.FromSeconds(20);
    private static readonly TimeSpan PostTunaFallbackProofSendWindow = TimeSpan.FromSeconds(12);
    private const string RegularV4ControlFeedbackPressureStallReason = "regular_v4_control_feedback_pressure";
    private const string PostTunaFallbackControlPlanePressureStallReason = "post_tuna_fallback_control_plane_pressure";
    ...
    private const int ReceiveStallFileTransferProtocolRepairGraceWindows = 6;
    private const int ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs = 20_000;
    ...
    private int postTunaFallbackUnprovenRecoveryEscalationArmed;
    private int postTunaFallbackUnprovenRecoveryEscalationUsed;
    private int postTunaFallbackUnprovenRecoveryEscalationUsedRecoveryCount = -1;
    private long postTunaFallbackUnprovenRecoveryEscalationArmedTick;
    private int postTunaFallbackUnprovenRecoveryEscalationPending;
    private long postTunaFallbackUnprovenRecoveryEscalationPendingTick;
    private string postTunaFallbackUnprovenRecoveryEscalationPendingReason = "post_tuna_fallback_state_refresh_failed";
    private long postTunaFallbackProofSendWindowExpiresTick;
    private string postTunaFallbackProofSendWindowReason = "none";
    private string postTunaFallbackProofSendWindowTrigger = "none";
    private string postTunaFallbackProofSendWindowSessionId = "none";
    private int regularV4UnprovenRecoveryEscalationUsedRecoveryCount = -1;
    private long lastBridgeTransportHealthSummaryTick;
    private long lastBridgeTransportHealthFramesSentSinceLast;
    private long lastBridgeTransportHealthMessagesReceivedSinceLast;
    ...
    private long lastBridgeSendFrameChannel;
    ...
    private PostTunaFallbackControlPlanePressureState? postTunaFallbackControlPlanePressure;
    ...
    private sealed class PostTunaFallbackControlPlanePressureState
    {
        public required string SessionId { get; init; }
    ...
    internal void MarkActiveFileTransferPostTunaFallbackRuntime(string transferId, string reason)
    {
        var normalizedTransferId = string.IsNullOrWhiteSpace(transferId) ? "(unknown)" : transferId.Trim();
        if (!activePostTunaFallbackRuntimeTransfers.TryAdd(normalizedTransferId, 0))
        {
            return;
        }

        Log(
            "event=filetransfer_active_runtime_post_tuna_fallback_marked; " +
        ...
        string authorityReason)
    {
        ...
        var shouldLog = false;
        var shouldResetEscalation = false;
        lock (gate)
        {
                ...
                current.LegGeneration != legGeneration)
            {
                ...
                shouldLog = true;
                shouldResetEscalation = true;
            }
            else if (liveRouteEpoch >= current.LiveRouteEpoch &&
                     transportEpoch >= current.TransportEpoch &&
                     bridgeRecoveryGeneration >= current.BridgeRecoveryGeneration &&
                     (!string.Equals(current.CheckpointRequestId ?? string.Empty, checkpointRequestId ?? string.Empty, StringComparison.Ordinal) ||
                      current.LiveRouteEpoch != liveRouteEpoch ||
                      current.TransportEpoch != transportEpoch ||
                      current.BridgeRecoveryGeneration != bridgeRecoveryGeneration ||
                      !string.Equals(current.AuthorityReason, normalizedReason, StringComparison.Ordinal)))
            {
                shouldResetEscalation = bridgeRecoveryGeneration != current.BridgeRecoveryGeneration;
                postTunaFallbackLegAuthority = new PostTunaFallbackLegAuthorityState
                {
                    SessionId = normalizedSessionId,
                    TransferId = normalizedTransferId,
                    ...
                    CheckpointRequestId = checkpointRequestId,
                    AuthorityReason = normalizedReason,
                    EscalationUsed = shouldResetEscalation ? false : current.EscalationUsed,
                };
                shouldLog = true;
            }
    ...
    internal void ClearActiveFileTransferPostTunaFallbackRuntime(string transferId, string reason)
    {
        ...
        lock (gate)
        {
            ...
            if (postTunaFallbackControlPlanePressure is { } pressure &&
                string.Equals(pressure.TransferId, normalizedTransferId, StringComparison.Ordinal))
            {
                postTunaFallbackControlPlanePressure = null;
            }
        }
    ...
    {
        var normalizedTransferId = string.IsNullOrWhiteSpace(transferId) ? "(unknown)" : transferId.Trim();
        var rawReason = string.IsNullOrWhiteSpace(reason)
            ? "regular_v4_control_feedback_pressure"
            : reason.Trim();
        var normalizedReason = SanitizeLogToken(rawReason);
        var activationSendOnly = string.Equals(
            rawReason,
            "regular_v4_sender_remote_frontier_pressure",
            StringComparison.Ordinal);
        var nowTick = Stopwatch.GetTimestamp();
        var ttl = activationSendOnly
            ? RegularV4ActivationSendPressureTtl
            : RegularV4ControlFeedbackPressureTtl;
        var expiresTick = AddStopwatchDuration(nowTick, ttl);
        var previousExpiresTick = activationSendOnly
            ? Interlocked.Exchange(ref regularV4ActivationSendPressureExpiresTick, expiresTick)
            : Interlocked.Exchange(ref regularV4ControlFeedbackPressureExpiresTick, expiresTick);
        ...
        Log(
            ...
            $"transfer_id={SanitizeLogToken(normalizedTransferId)}; reason={normalizedReason}; activation_send_only={(activationSendOnly ? 1 : 0)}; credit_exhausted_time_ms={Math.Max(0, creditExhaustedTimeMs)}; frontier_lag_chunks={Math.Max(0, frontierLagChunks)}; pending_repair_count={Math.Max(0, pendingRepairCount)}; ttl_ms={(long)ttl.TotalMilliseconds}; " +
    ...
    internal void ReportPostTunaFallbackControlPlanePressure(
        string sessionId,
        string transferId,
        string? routeToken,
        int protocolVersion,
        ...
        string? reason)
    {
        ...
        var normalizedSessionId = string.IsNullOrWhiteSpace(sessionId) ? "none" : sessionId.Trim();
        var normalizedTransferId = string.IsNullOrWhiteSpace(transferId) ? "(unknown)" : transferId.Trim();
        var normalizedKind = SanitizeLogToken(string.IsNullOrWhiteSpace(kind) ? "control_plane" : kind);
        var normalizedReason = SanitizeLogToken(string.IsNullOrWhiteSpace(reason) ? "post_tuna_fallback_control_plane_pressure" : reason);
        var normalizedCheckpointRequestId = string.IsNullOrWhiteSpace(checkpointRequestId) ? null : checkpointRequestId.Trim();
        var nowTick = Stopwatch.GetTimestamp();
        var expiresTick = AddStopwatchDuration(nowTick, PostTunaFallbackControlPlanePressureTtl);
        var shouldLogMarked = false;
        var authoritySource = string.Empty;
        var staleReason = string.Empty;

        lock (gate)
        {
            var hasAuthority = TryGetCurrentPostTunaFallbackLegAuthorityUnsafe(out var authority);
            var authorityMatchesSessionTransfer =
                hasAuthority &&
                string.Equals(authority.SessionId, normalizedSessionId, StringComparison.Ordinal) &&
                string.Equals(authority.TransferId, normalizedTransferId, StringComparison.Ordinal);
            var hasExactAuthority =
                authorityMatchesSessionTransfer &&
                authority.LegGeneration == legGeneration &&
                authority.LiveRouteEpoch == liveRouteEpoch &&
                authority.TransportEpoch == transportEpoch &&
                authority.BridgeRecoveryGeneration == bridgeRecoveryGeneration &&
                (authority.CheckpointRequestId is null ||
                 normalizedCheckpointRequestId is null ||
                 string.Equals(authority.CheckpointRequestId, normalizedCheckpointRequestId, StringComparison.Ordinal));
            var hasCurrentRuntime =
                activePostTunaFallbackRuntimeTransfers.ContainsKey(normalizedTransferId) &&
                Volatile.Read(ref activeFileTransferDataSessions) > 0;

            if (hasExactAuthority)
            {
                authoritySource = "bridge_authority";
            }
            else if (hasCurrentRuntime && !authorityMatchesSessionTransfer)
            {
                authoritySource = "active_runtime";
            }
            else
            {
                staleReason = "authority_mismatch";
            }

            if (string.IsNullOrEmpty(staleReason) &&
                postTunaFallbackControlPlanePressure is { } current &&
                     string.Equals(current.SessionId, normalizedSessionId, StringComparison.Ordinal) &&
                     string.Equals(current.TransferId, normalizedTransferId, StringComparison.Ordinal) &&
                     current.LegGeneration == legGeneration &&
                     current.BridgeRecoveryGeneration == bridgeRecoveryGeneration &&
                     current.TransportEpoch == transportEpoch &&
                     string.Equals(current.CheckpointRequestId ?? string.Empty, normalizedCheckpointRequestId ?? string.Empty, StringComparison.Ordinal))
            {
                current.ExpiresTick = expiresTick;
            }
            else if (string.IsNullOrEmpty(staleReason))
            {
                postTunaFallbackControlPlanePressure = new PostTunaFallbackControlPlanePressureState
                {
                    SessionId = normalizedSessionId,
                    TransferId = normalizedTransferId,
                    ...
                    BridgeRecoveryGeneration = bridgeRecoveryGeneration,
                    TransportEpoch = transportEpoch,
                    CheckpointRequestId = normalizedCheckpointRequestId,
                    Kind = normalizedKind,
        ...
        if (!string.IsNullOrEmpty(staleReason))
        {
            Log(
                ...
                $"reason={staleReason}; session_id={SanitizeLogToken(normalizedSessionId)}; transfer_id={SanitizeLogToken(normalizedTransferId)}; route={SanitizeLogToken(routeToken ?? "(none)")}; protocol_version={protocolVersion}; live_route_epoch={liveRouteEpoch}; leg_generation={legGeneration}; bridge_recovery_generation={bridgeRecoveryGeneration}; transport_epoch={transportEpoch}; checkpoint_request_id={SanitizeLogToken(normalizedCheckpointRequestId ?? "none")}; kind={normalizedKind}; pressure_reason={normalizedReason}");
            return;
        }
        ...
        Log(
            ...
            $"session_id={SanitizeLogToken(normalizedSessionId)}; transfer_id={SanitizeLogToken(normalizedTransferId)}; route=post_tuna_fallback_v6; protocol_version={protocolVersion}; live_route_epoch={liveRouteEpoch}; leg_generation={legGeneration}; bridge_recovery_generation={bridgeRecoveryGeneration}; transport_epoch={transportEpoch}; checkpoint_request_id={SanitizeLogToken(normalizedCheckpointRequestId ?? "none")}; kind={normalizedKind}; reason={normalizedReason}; authority_source={authoritySource}; ttl_ms={(long)PostTunaFallbackControlPlanePressureTtl.TotalMilliseconds}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}; active_post_tuna_fallback_runtime_sessions={activePostTunaFallbackRuntimeTransfers.Count}");
    }

    internal void ClearPostTunaFallbackControlPlanePressure(string transferId, string reason)
    {
        var normalizedTransferId = string.IsNullOrWhiteSpace(transferId) ? "(unknown)" : transferId.Trim();
        PostTunaFallbackControlPlanePressureState? cleared = null;
        lock (gate)
        {
            if (postTunaFallbackControlPlanePressure is { } pressure &&
                (string.Equals(pressure.TransferId, normalizedTransferId, StringComparison.Ordinal) ||
                 string.Equals(normalizedTransferId, "(all)", StringComparison.Ordinal)))
            {
                cleared = pressure;
                postTunaFallbackControlPlanePressure = null;
            }
        ...
        var activeMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(cleared.CreatedTick, Stopwatch.GetTimestamp()).TotalMilliseconds);
        Log(
            "event=filetransfer_post_tuna_fallback_control_plane_pressure_cleared; " +
            $"reason={SanitizeLogToken(reason)}; session_id={SanitizeLogToken(cleared.SessionId)}; transfer_id={SanitizeLogToken(cleared.TransferId)}; route={SanitizeLogToken(cleared.RouteToken)}; protocol_version={cleared.ProtocolVersion}; live_route_epoch={cleared.LiveRouteEpoch}; leg_generation={cleared.LegGeneration}; bridge_recovery_generation={cleared.BridgeRecoveryGeneration}; transport_epoch={cleared.TransportEpoch}; checkpoint_request_id={SanitizeLogToken(cleared.CheckpointRequestId ?? "none")}; pressure_kind={SanitizeLogToken(cleared.Kind)}; pressure_reason={SanitizeLogToken(cleared.Reason)}; recovery_attempted={(cleared.RecoveryAttempted ? 1 : 0)}; active_ms={activeMs}");
    }
    ...
    internal void ArmPostTunaFallbackProofSendWindow(string reason, string trigger, string? sessionId)
    {
        ...
        lock (gate)
        {
            postTunaFallbackProofSendWindowReason = normalizedReason;
            postTunaFallbackProofSendWindowTrigger = normalizedTrigger;
            postTunaFallbackProofSendWindowSessionId = normalizedSessionId;
        }

        Log(
            "event=nkn_bridge_post_tuna_fallback_proof_send_window_armed; " +
            $"reason={normalizedReason}; trigger={normalizedTrigger}; session_id={normalizedSessionId}; window_ms={(long)PostTunaFallbackProofSendWindow.TotalMilliseconds}; " +
            $"active_post_tuna_fallback_runtime_sessions={activePostTunaFallbackRuntimeTransfers.Count}; active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
    }
    ...
    internal void ClearActiveFileTransferRuntimeTransfers(string reason)
    {
        ...
        Log(
            ...
            $"reason={SanitizeLogToken(reason)}; cleared_count={clearedCount}; cleared_post_tuna_fallback_count={clearedPostTunaFallbackCount}; active_file_transfer_data_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}");
    ...
    internal bool RequestFileTransferReceiveStallRecovery(string reason)
    {
        var normalizedReason = string.IsNullOrWhiteSpace(reason)
            ...
            : SanitizeLogToken(reason);
        var postTunaFallbackRecoveryRequest = IsPostTunaFallbackReceiveStallRecoveryReason(normalizedReason);
        var regularV4RuntimeUnlockRecoveryRequest = IsRegularV4RuntimeUnlockReceiveStallRecoveryReason(normalizedReason);
        var nowTick = Stopwatch.GetTimestamp();
        if (postTunaFallbackRecoveryRequest)
        {
            ArmPostTunaFallbackUnprovenRecoveryEscalation(normalizedReason, nowTick);
            TryClearCompletedUnprovenPostTunaFallbackRecoveryGate(normalizedReason, "core_request_enter");
        }
        else if (regularV4RuntimeUnlockRecoveryRequest)
        {
            TryClearCompletedUnprovenRegularV4RecoveryGate(normalizedReason, "core_request_enter");
        }
        ...
        {
            var enteredAfterClearingStaleGate =
                (postTunaFallbackRecoveryRequest &&
                 TryClearCompletedUnprovenPostTunaFallbackRecoveryGate(normalizedReason, "core_request_already_in_progress") ||
                 regularV4RuntimeUnlockRecoveryRequest &&
                 TryClearCompletedUnprovenRegularV4RecoveryGate(normalizedReason, "core_request_already_in_progress")) &&
                Interlocked.CompareExchange(ref receiveStallRecoveryInProgress, 1, 0) == 0;
            if (!enteredAfterClearingStaleGate)
            {
                if (postTunaFallbackRecoveryRequest &&
                    ArmPendingPostTunaFallbackUnprovenRecoveryEscalation(
                        normalizedReason,
                        "core_request_already_in_progress",
                        nowTick))
                {
                    EmitReceiveStallRecoveryDeferredLifecycle(
                        "post_tuna_fallback_unproven_escalation_pending",
                        normalizedReason,
        ...
        var postTunaFallbackUnprovenEscalation = postTunaFallbackRecoveryRequest &&
            TryConsumePostTunaFallbackUnprovenRecoveryEscalation(
                "core_request",
                normalizedReason,
                normalizedReason,
                "core_filetransfer_request",
                ...
                0,
                0);
        var regularV4UnprovenEscalation = regularV4RuntimeUnlockRecoveryRequest &&
            TryConsumeRegularV4UnprovenRecoveryEscalation(
                "core_request",
                normalizedReason,
                "core_filetransfer_request",
                0,
        ...
        if (lastRecoveryTick > 0 &&
            ...
            !postTunaFallbackUnprovenEscalation &&
            !regularV4UnprovenEscalation)
        {
            ...
            EmitReceiveStallRecoveryDeferredLifecycle(
                "active_filetransfer_cooldown",
                normalizedReason,
                "core_filetransfer_request");
            return false;
        }

        if (postTunaFallbackUnprovenEscalation &&
            lastRecoveryTick > 0 &&
            Stopwatch.GetElapsedTime(lastRecoveryTick, nowTick) < requestCooldown)
        {
            var elapsedSinceLastRecoveryMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(lastRecoveryTick, nowTick).TotalMilliseconds);
            Log(
                "event=nkn_bridge_receive_stall_recovery_cooldown_bypassed; reason=post_tuna_fallback_unproven_escalation; " +
                $"requested_reason={normalizedReason}; elapsed_since_last_recovery_ms={elapsedSinceLastRecoveryMs}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}");
        }
        else if (regularV4UnprovenEscalation &&
            lastRecoveryTick > 0 &&
            Stopwatch.GetElapsedTime(lastRecoveryTick, nowTick) < requestCooldown)
        {
            var elapsedSinceLastRecoveryMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(lastRecoveryTick, nowTick).TotalMilliseconds);
            Log(
                "event=nkn_bridge_receive_stall_recovery_cooldown_bypassed; reason=regular_v4_unproven_escalation; " +
                $"requested_reason={normalizedReason}; elapsed_since_last_recovery_ms={elapsedSinceLastRecoveryMs}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}");
        }
        ...
        var recoveryReason = postTunaFallbackUnprovenEscalation
            ? "post_tuna_fallback_unproven_recovery_escalation"
            : regularV4UnprovenEscalation
        ...
        Log(
            ...
            $"requested_reason={normalizedReason}; stall_reason={recoveryReason}; post_tuna_fallback_unproven_escalation={(postTunaFallbackUnprovenEscalation ? 1 : 0)}; regular_v4_runtime_unlock_unproven_escalation={(regularV4UnprovenEscalation ? 1 : 0)}; attempt={attempt}; active_file_transfer_sessions={activeDataSessionCount}; active_file_transfer_runtime_sessions={activeRuntimeCount}");
        _ = Task.Run(
    ...
    {
        ClearPostTunaFallbackUnprovenRecoveryEscalation(reason);
        Interlocked.Exchange(ref postTunaFallbackProofSendWindowExpiresTick, 0);
        lock (gate)
        {
            postTunaFallbackControlPlanePressure = null;
        }
        Volatile.Write(ref regularV4UnprovenRecoveryEscalationUsedRecoveryCount, -1);
        var nowTick = Stopwatch.GetTimestamp();
        var previousPressureExpiresTick = Interlocked.Exchange(ref regularV4ControlFeedbackPressureExpiresTick, 0);
        var previousActivationPressureExpiresTick = Interlocked.Exchange(ref regularV4ActivationSendPressureExpiresTick, 0);
        var previousPressureActive = previousPressureExpiresTick > nowTick;
        var previousActivationPressureActive = previousActivationPressureExpiresTick > nowTick;
        ...
        if (previousAllZeroWindows <= 0 &&
            ...
            !previousActivationPressureActive)
        {
            return;
        }
        ...
        Log(
            ...
            $"previous_regular_v4_control_feedback_pressure_active={(previousPressureActive ? 1 : 0)}; previous_regular_v4_activation_send_pressure_active={(previousActivationPressureActive ? 1 : 0)}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
    }
    ...
    private bool TryGetPostTunaFallbackControlPlanePressure(
        long nowTick,
        out PostTunaFallbackControlPlanePressureState pressure,
        out long remainingMs)
    {
        ...
        lock (gate)
        {
            if (postTunaFallbackControlPlanePressure is not { } current)
            {
                return false;
            }

            var hasExactAuthority =
                TryGetCurrentPostTunaFallbackLegAuthorityUnsafe(out var authority) &&
                string.Equals(authority.SessionId, current.SessionId, StringComparison.Ordinal) &&
                string.Equals(authority.TransferId, current.TransferId, StringComparison.Ordinal) &&
                authority.LegGeneration == current.LegGeneration &&
                authority.LiveRouteEpoch == current.LiveRouteEpoch &&
                authority.TransportEpoch == current.TransportEpoch &&
                authority.BridgeRecoveryGeneration == current.BridgeRecoveryGeneration;
            var hasCurrentRuntime =
                string.Equals(current.AuthoritySource, "active_runtime", StringComparison.Ordinal) &&
                activePostTunaFallbackRuntimeTransfers.ContainsKey(current.TransferId) &&
                Volatile.Read(ref activeFileTransferDataSessions) > 0;

            if (current.ExpiresTick <= nowTick ||
                (!hasExactAuthority && !hasCurrentRuntime))
            {
                postTunaFallbackControlPlanePressure = null;
                return false;
            }

            remainingMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(nowTick, current.ExpiresTick).TotalMilliseconds);
            pressure = current;
            return true;
        }
    }

    private bool TryConsumePostTunaFallbackControlPlanePressureRecovery(
        long nowTick,
        out PostTunaFallbackControlPlanePressureState pressure,
        out long remainingMs)
    {
        if (!TryGetPostTunaFallbackControlPlanePressure(nowTick, out pressure, out remainingMs))
        {
            return false;
        }
        ...
        {
            if (postTunaFallbackControlPlanePressure is not { } current ||
                !ReferenceEquals(current, pressure))
            {
                return false;
            }

            if (current.RecoveryAttempted)
            {
                return false;
            }
    ...
    private bool IsPostTunaFallbackProofSendWindowActive(
        long nowTick,
        out long remainingMs,
        out string reason,
        out string trigger,
        out string sessionId)
    {
        ...
        trigger = "none";
        sessionId = "none";
        var expiresTick = Volatile.Read(ref postTunaFallbackProofSendWindowExpiresTick);
        if (expiresTick <= nowTick)
        {
            if (expiresTick > 0)
            {
                Interlocked.CompareExchange(ref postTunaFallbackProofSendWindowExpiresTick, 0, expiresTick);
            }
        ...
        lock (gate)
        {
            reason = postTunaFallbackProofSendWindowReason;
            trigger = postTunaFallbackProofSendWindowTrigger;
            sessionId = postTunaFallbackProofSendWindowSessionId;
        }
    ...
    private static string FormatPostTunaFallbackControlPlanePressureFields(PostTunaFallbackControlPlanePressureState? pressure)
    {
        if (pressure is null)
        {
            return string.Empty;
        ...
        return
            $"; session_id={SanitizeLogToken(pressure.SessionId)}" +
            $"; transfer_id={SanitizeLogToken(pressure.TransferId)}" +
            $"; route={SanitizeLogToken(pressure.RouteToken)}" +
            $"; protocol_version={pressure.ProtocolVersion}" +
            $"; live_route_epoch={pressure.LiveRouteEpoch}" +
            $"; leg_generation={pressure.LegGeneration}" +
            $"; transport_epoch={pressure.TransportEpoch}" +
            $"; bridge_recovery_generation={pressure.BridgeRecoveryGeneration}" +
            $"; checkpoint_request_id={SanitizeLogToken(pressure.CheckpointRequestId ?? "none")}" +
            $"; pressure_kind={SanitizeLogToken(pressure.Kind)}" +
            $"; pressure_reason={SanitizeLogToken(pressure.Reason)}" +
            $"; pressure_authority_source={SanitizeLogToken(pressure.AuthoritySource)}" +
            $"; pressure_recovery_attempted={(pressure.RecoveryAttempted ? 1 : 0)}";
    }
    ...
    private bool HasCompletedPostTunaFallbackRecoveryWithoutReceiveProof(long startedTick, long completedTick)
    {
        if (Volatile.Read(ref receiveStallRecoveryInProgress) == 0 ||
            startedTick <= 0 ||
            completedTick <= 0 ||
            startedTick > completedTick)
        {
            return false;
        }

        return Volatile.Read(ref receiveStallRecoveryRequiresControlProof) != 0 ||
               Volatile.Read(ref receiveStallRecoveryRequiresBulkProof) != 0;
    }

    private bool HasPostTunaFallbackUnprovenRecoveryCandidate()
    {
        ...
        return HasCompletedPostTunaFallbackRecoveryWithoutReceiveProof(
            Volatile.Read(ref receiveStallLastRecoveryStartedTick),
            Volatile.Read(ref receiveStallLastRecoveryCompletedTick));
    }
    ...
    private void ArmPostTunaFallbackUnprovenRecoveryEscalation(string requestedReason, long nowTick)
    {
        ...
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationArmed, 1);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationArmedTick, nowTick);
        Log(
            "event=nkn_bridge_receive_stall_recovery_post_tuna_fallback_unproven_escalation_armed; " +
            $"requested_reason={SanitizeLogToken(requestedReason)}; awaiting_receive_proof={Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof)}; recovery_in_progress={Volatile.Read(ref receiveStallRecoveryInProgress)}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
    }

    private bool ArmPendingPostTunaFallbackUnprovenRecoveryEscalation(
        string requestedReason,
        string trigger,
        long nowTick)
    {
        if (!IsPostTunaFallbackReceiveStallRecoveryReason(requestedReason) ||
            Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions)) <= 0 &&
            activeFileTransferRuntimeTransfers.IsEmpty)
        {
            return false;
        ...
        lock (gate)
        {
            postTunaFallbackUnprovenRecoveryEscalationPendingReason = SanitizeLogToken(requestedReason);
        }

        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationPending, 1);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationPendingTick, nowTick);
        Log(
            "event=nkn_bridge_receive_stall_recovery_post_tuna_fallback_unproven_escalation_pending; " +
            $"trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; awaiting_receive_proof={Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof)}; recovery_in_progress={Volatile.Read(ref receiveStallRecoveryInProgress)}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
        return true;
    }

    private bool TryClearCompletedUnprovenPostTunaFallbackRecoveryGate(string requestedReason, string trigger)
    {
        if (!IsPostTunaFallbackReceiveStallRecoveryReason(requestedReason) &&
            Volatile.Read(ref postTunaFallbackUnprovenRecoveryEscalationArmed) == 0)
        {
            return false;
        }

        var startedTick = Volatile.Read(ref receiveStallLastRecoveryStartedTick);
        var completedTick = Volatile.Read(ref receiveStallLastRecoveryCompletedTick);
        if (Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof) == 0 &&
            !HasCompletedPostTunaFallbackRecoveryWithoutReceiveProof(startedTick, completedTick))
        {
            return TryClearStalePostTunaFallbackRecoveryInProgressGate(requestedReason, trigger, startedTick, completedTick);
        }

        if (completedTick <= 0 || startedTick > completedTick)
        {
            return TryClearStalePostTunaFallbackRecoveryInProgressGate(requestedReason, trigger, startedTick, completedTick);
        }

        if (Interlocked.CompareExchange(ref receiveStallRecoveryInProgress, 0, 1) != 1)
        {
            return TryClearStalePostTunaFallbackRecoveryInProgressGate(requestedReason, trigger, startedTick, completedTick);
        }

        var completedAgeMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(completedTick).TotalMilliseconds);
        Log(
            "event=nkn_bridge_receive_stall_recovery_stale_gate_cleared; " +
            $"reason=post_tuna_fallback_unproven_recovery; trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; completed_age_ms={completedAgeMs}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
        return true;
    }

    private bool TryClearCompletedUnprovenRegularV4RecoveryGate(string requestedReason, string trigger)
    {
        if (!IsRegularV4RuntimeUnlockReceiveStallRecoveryReason(requestedReason))
        {
            return false;
        }
        ...
        if (completedTick <= 0 || startedTick > completedTick)
        {
            return TryClearStaleRegularV4RecoveryInProgressGate(requestedReason, trigger, startedTick, completedTick);
        }

        if (!HasPostTunaFallbackUnprovenRecoveryCandidate())
        {
            return false;
        }
        ...
        Log(
            "event=nkn_bridge_receive_stall_recovery_stale_gate_cleared; " +
            $"reason=regular_v4_unproven_recovery; trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; completed_age_ms={completedAgeMs}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
        return true;
    }

    private bool TryClearStaleRegularV4RecoveryInProgressGate(
        string requestedReason,
        string trigger,
        long startedTick,
        long completedTick)
    {
        ...
        var age = Stopwatch.GetElapsedTime(startedTick);
        if (age < ReceiveStallActiveFileTransferUnprovenRecoveryCooldown)
        {
            return false;
        }
        ...
        Volatile.Write(ref receiveStallRecoveryAwaitingReceiveProof, 1);
        Volatile.Write(ref receiveStallLastRecoveryCompletedTick, Stopwatch.GetTimestamp());
        var ageMs = Math.Max(0, (long)age.TotalMilliseconds);
        Log(
            "event=nkn_bridge_receive_stall_recovery_stale_gate_cleared; " +
            $"reason=regular_v4_stale_in_progress_recovery; trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; started_age_ms={ageMs}; completed_tick_present={(completedTick > 0 ? 1 : 0)}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; awaiting_receive_proof={Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof)}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
        return true;
    }

    private bool TryClearStalePostTunaFallbackRecoveryInProgressGate(
        string requestedReason,
        string trigger,
        long startedTick,
        long completedTick)
    {
        ...
        Log(
            "event=nkn_bridge_receive_stall_recovery_stale_gate_cleared; " +
            $"reason=post_tuna_fallback_stale_in_progress_recovery; trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; started_age_ms={ageMs}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; awaiting_receive_proof={Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof)}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
        return true;
    }

    private bool TryConsumePostTunaFallbackUnprovenRecoveryEscalation(
        string trigger,
        string requestedReason,
        string stallReason,
        string connectKey,
        int consecutiveWindows,
        int bulkConsecutiveWindows,
        int controlConsecutiveWindows)
    {
        if (Volatile.Read(ref postTunaFallbackUnprovenRecoveryEscalationArmed) == 0)
        {
            return false;
        }
        ...
        if (consumedRecoveryCount == recoveryCount ||
            Interlocked.CompareExchange(
                ref postTunaFallbackUnprovenRecoveryEscalationUsedRecoveryCount,
                recoveryCount,
        ...
        Interlocked.Exchange(ref postTunaFallbackUnprovenRecoveryEscalationPending, 0);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationPendingTick, 0);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationArmed, 0);
        TryClearCompletedUnprovenPostTunaFallbackRecoveryGate(requestedReason, trigger);
        var completedAgeMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(completedTick).TotalMilliseconds);
        var armedTick = Volatile.Read(ref postTunaFallbackUnprovenRecoveryEscalationArmedTick);
        var armedAgeMs = armedTick > 0
            ? Math.Max(0, (long)Stopwatch.GetElapsedTime(armedTick).TotalMilliseconds)
            : -1;
        Log(
    ...
    private bool TryArmPostTunaFallbackUnprovenRecoveryEscalationForAutomaticStall(
        string trigger,
        string stallReason,
        string connectKey,
        int consecutiveWindows,
    ...
    {
        if (activePostTunaFallbackRuntimeTransfers.IsEmpty ||
            !HasPostTunaFallbackUnprovenRecoveryCandidate() ||
            Volatile.Read(ref receiveStallLastRecoveryCompletedTick) <= 0 ||
            Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions)) <= 0 && activeFileTransferRuntimeTransfers.IsEmpty)
        {
            return false;
        }
        ...
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationArmedTick, nowTick);
        Log(
            "event=nkn_bridge_receive_stall_recovery_post_tuna_fallback_unproven_escalation_auto_armed; " +
            $"trigger={SanitizeLogToken(trigger)}; requested_reason=post_tuna_fallback_peer_silence; connect_key={SanitizeLogToken(connectKey)}; stall_reason={SanitizeLogToken(stallReason)}; awaiting_receive_proof={Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof)}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
            $"consecutive_zero_receive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; active_post_tuna_fallback_runtime_sessions={activePostTunaFallbackRuntimeTransfers.Count}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
        return true;
    }

    private bool TryConsumeRegularV4UnprovenRecoveryEscalation(
        string trigger,
        string stallReason,
        string connectKey,
        int consecutiveWindows,
        ...
        int controlConsecutiveWindows)
    {
        var startedTick = Volatile.Read(ref receiveStallLastRecoveryStartedTick);
        var completedTick = Volatile.Read(ref receiveStallLastRecoveryCompletedTick);
        var completedAwaitingProof =
            Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof) != 0 &&
            completedTick > 0;
        var staleInProgressCleared = TryClearStaleRegularV4RecoveryInProgressGate(
            "regular_v4_unproven_recovery_escalation",
            trigger,
            startedTick,
            completedTick);
        if (!completedAwaitingProof &&
            !staleInProgressCleared ||
            Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions)) <= 0 && activeFileTransferRuntimeTransfers.IsEmpty)
        {
            return false;
        }
        ...
        var recoveryCount = Volatile.Read(ref receiveStallRecoveryCount);
        var consumedRecoveryCount = Volatile.Read(ref regularV4UnprovenRecoveryEscalationUsedRecoveryCount);
        if (consumedRecoveryCount == recoveryCount ||
            Interlocked.CompareExchange(
                ref regularV4UnprovenRecoveryEscalationUsedRecoveryCount,
                recoveryCount,
                consumedRecoveryCount) != consumedRecoveryCount)
        {
            return false;
        ...
        var completedAgeMs = completedTick > 0
            ? Math.Max(0, (long)Stopwatch.GetElapsedTime(completedTick).TotalMilliseconds)
            : -1;
        var startedAgeMs = startedTick > 0
            ? Math.Max(0, (long)Stopwatch.GetElapsedTime(startedTick).TotalMilliseconds)
            : -1;
        Log(
            "event=nkn_bridge_receive_stall_recovery_regular_v4_unproven_escalation_allowed; " +
            $"trigger={SanitizeLogToken(trigger)}; connect_key={SanitizeLogToken(connectKey)}; stall_reason={SanitizeLogToken(stallReason)}; completed_age_ms={completedAgeMs}; started_age_ms={startedAgeMs}; stale_in_progress_cleared={(staleInProgressCleared ? 1 : 0)}; recovery_count={recoveryCount}; " +
            $"consecutive_zero_receive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; " +
            $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
        return true;
    }

    private void ClearPostTunaFallbackUnprovenRecoveryEscalation(string reason)
    {
        var wasArmed = Interlocked.Exchange(ref postTunaFallbackUnprovenRecoveryEscalationArmed, 0);
        var wasUsed = Interlocked.Exchange(ref postTunaFallbackUnprovenRecoveryEscalationUsed, 0);
        var wasPending = Interlocked.Exchange(ref postTunaFallbackUnprovenRecoveryEscalationPending, 0);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationUsedRecoveryCount, -1);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationArmedTick, 0);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationPendingTick, 0);
        if (wasArmed == 0 && wasUsed == 0 && wasPending == 0)
        {
            return;
        }

        Log(
            "event=nkn_bridge_receive_stall_recovery_post_tuna_fallback_unproven_escalation_cleared; " +
            $"reason={SanitizeLogToken(reason)}; was_armed={wasArmed}; was_used={wasUsed}; was_pending={wasPending}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}");
    }

    private void TrySchedulePendingPostTunaFallbackUnprovenRecoveryEscalation(string trigger)
    {
        if (Interlocked.CompareExchange(ref postTunaFallbackUnprovenRecoveryEscalationPending, 0, 1) != 1)
        {
            return;
        }
        ...
        lock (gate)
        {
            requestedReason = postTunaFallbackUnprovenRecoveryEscalationPendingReason;
            postTunaFallbackUnprovenRecoveryEscalationPendingReason = "post_tuna_fallback_state_refresh_failed";
        }

        var pendingTick = Volatile.Read(ref postTunaFallbackUnprovenRecoveryEscalationPendingTick);
        Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationPendingTick, 0);
        var activeDataSessionCount = Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions));
        var activeRuntimeCount = activeFileTransferRuntimeTransfers.Count;
        if (disposed ||
            ...
            !HasPostTunaFallbackUnprovenRecoveryCandidate() ||
            Volatile.Read(ref receiveStallLastRecoveryCompletedTick) <= 0 ||
            activeDataSessionCount <= 0 && activeRuntimeCount <= 0)
        {
            Log(
                "event=nkn_bridge_receive_stall_recovery_post_tuna_fallback_unproven_escalation_pending_dropped; " +
                $"trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; awaiting_receive_proof={Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof)}; recovery_in_progress={Volatile.Read(ref receiveStallRecoveryInProgress)}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                $"active_file_transfer_sessions={activeDataSessionCount}; active_file_transfer_runtime_sessions={activeRuntimeCount}");
            return;
        }

        var pendingAgeMs = pendingTick > 0
            ? Math.Max(0, (long)Stopwatch.GetElapsedTime(pendingTick).TotalMilliseconds)
            : -1;
        Log(
            "event=nkn_bridge_receive_stall_recovery_post_tuna_fallback_unproven_escalation_pending_scheduled; " +
            $"trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; pending_age_ms={pendingAgeMs}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
            $"active_file_transfer_sessions={activeDataSessionCount}; active_file_transfer_runtime_sessions={activeRuntimeCount}");
        _ = Task.Run(
            () =>
            {
                try
                {
                    RequestFileTransferReceiveStallRecovery(requestedReason);
                }
                catch (Exception ex) when (ex is not OperationCanceledException)
                {
                    Log(
                        "event=nkn_bridge_receive_stall_recovery_post_tuna_fallback_unproven_escalation_pending_failed; " +
                        $"trigger={SanitizeLogToken(trigger)}; requested_reason={SanitizeLogToken(requestedReason)}; reason={ex.GetType().Name}; message={SanitizeLogToken(ex.Message)}");
                }
            },
            CancellationToken.None);
    }

    private bool IsRegularV4ActivationSendPressureActive(long nowTick, out long remainingMs)
    {
        var expiresTick = Volatile.Read(ref regularV4ActivationSendPressureExpiresTick);
        if (expiresTick <= nowTick)
        {
            remainingMs = 0;
            return false;
    ...
    internal bool TryGetFileTransferRegularV4ActivationSendBlocker(out string reason, out long remainingMs)
        => TryGetFileTransferRegularV4ActivationSendBlocker(
            out reason,
            out remainingMs,
            includeRegularV4Pressure: true);

    internal bool TryGetFileTransferRegularV4ActivationSendPressure(out string reason, out long remainingMs)
    {
        reason = string.Empty;
        remainingMs = 0;
        ...
        var nowTick = Stopwatch.GetTimestamp();
        if (IsRegularV4ControlFeedbackPressureActive(nowTick, out remainingMs))
        {
            reason = "regular_v4_control_feedback_pressure";
            return true;
        }

        if (IsRegularV4ActivationSendPressureActive(nowTick, out remainingMs))
        {
            reason = "regular_v4_activation_send_pressure";
            return true;
        }
    ...
    internal bool HasActiveFileTransferRuntimeForActivationSend()
        => Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions)) > 0 ||
           !activeFileTransferRuntimeTransfers.IsEmpty;

    internal bool TryGetFileTransferRegularV4ActivationSendBlocker(
        out string reason,
        out long remainingMs,
        bool includeRegularV4Pressure)
    {
        reason = string.Empty;
        remainingMs = 0;
        ...
        if (Volatile.Read(ref receiveStallRecoveryInProgress) != 0)
        {
            reason = "receive_stall_recovery_in_progress";
            return true;
        }

        if (Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof) != 0)
        {
            reason = "receive_stall_recovery_awaiting_receive_proof";
            return true;
        }

        var nowTick = Stopwatch.GetTimestamp();
        if (includeRegularV4Pressure &&
            IsRegularV4ControlFeedbackPressureActive(nowTick, out remainingMs))
        {
            reason = "regular_v4_control_feedback_pressure";
            return true;
        }
        ...
        if (includeRegularV4Pressure &&
            IsRegularV4ActivationSendPressureActive(nowTick, out remainingMs))
        {
            reason = "regular_v4_activation_send_pressure";
            return true;
        }

        var allZeroWindows = Volatile.Read(ref receiveStallConsecutiveWindows);
        if (allZeroWindows > 0)
        {
            reason = "all_zero_receive_window";
            return true;
        }

        var controlZeroWindows = Volatile.Read(ref receiveStallControlConsecutiveWindows);
        if (controlZeroWindows > 0)
        {
            reason = "control_zero_receive_window";
            return true;
        }

        var bulkZeroWindows = Volatile.Read(ref receiveStallBulkConsecutiveWindows);
        if (bulkZeroWindows > 0)
        {
            reason = "bulk_zero_receive_window";
            return true;
        }

        var lastHealthTick = Volatile.Read(ref lastBridgeTransportHealthSummaryTick);
        if (lastHealthTick <= 0 ||
            Stopwatch.GetElapsedTime(lastHealthTick, nowTick) > ScreenShareBridgeLogInterval + ScreenShareBridgeLogInterval)
        {
            return false;
        }

        var messagesSinceLast = Volatile.Read(ref lastBridgeTransportHealthMessagesReceivedSinceLast);
        var controlLastReceivedAgeMs = Volatile.Read(ref lastBridgeTransportHealthControlLastReceivedAgeMs);
        var bulkLastReceivedAgeMs = Volatile.Read(ref lastBridgeTransportHealthBulkLastReceivedAgeMs);
    ...
    public async Task DisconnectAsync()
    {
        ...
        finally
        {
            ...
            ClearPostTunaFallbackUnprovenRecoveryEscalation("disconnect");
    ...
    private async Task SendCoreAsync(string destination, byte[] payload, NknBridgeChannel channel, CancellationToken ct)
    {
        ...
        Volatile.Write(ref lastBridgeSendFrameTick, Stopwatch.GetTimestamp());
        Volatile.Write(ref lastBridgeSendFramePayloadBytes, payload.Length);
        Volatile.Write(ref lastBridgeSendFrameSerializedBytes, serializedBytes);
        Volatile.Write(ref lastBridgeSendFrameChannel, (long)channel);
    ...
    private void HandleBridgeTransportHealthSummary(JsonElement root)
    {
        ...
        var bulkSendMode = TryGetString(root, "bulk_send_mode", out var bulkSendModeValue) ? NormalizeFileTransferBulkSendMode(bulkSendModeValue) : "(unknown)";
        ...
        Log(
            ...
            $"ldr={latestDisconnectReason}; csc={controlSubClients}; msc={mediaSubClients}; bsc={bulkSubClients}; bcc={bulkSendConcurrency}; bsm={bulkSendMode}");
        ...
        long sampleWindowMs)
    {
        ...
        var postTunaFallbackControlPlanePressureActive = TryGetPostTunaFallbackControlPlanePressure(
            Stopwatch.GetTimestamp(),
            out var postTunaFallbackControlPlanePressure,
            out var postTunaFallbackControlPlanePressureRemainingMs);
        var awaitingReceiveProofForRegularV4Pressure =
            Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof) != 0;
        var regularV4ControlFeedbackPressureRecoveryNeeded =
            fileTransferActive &&
            regularV4ControlFeedbackPressureActive &&
            !awaitingReceiveProofForRegularV4Pressure &&
            controlMessagesReceivedSinceLast == 0 &&
            bulkMessagesReceivedSinceLast == 0 &&
        ...
        Log(
            ...
            $"filetransfer_fast_recovery_enabled={(options.ReceiveStallFileTransferFastRecoveryEnabled ? 1 : 0)}; regular_v4_control_feedback_pressure={(regularV4ControlFeedbackPressureActive ? 1 : 0)}; regular_v4_control_feedback_pressure_remaining_ms={regularV4ControlFeedbackPressureRemainingMs}; post_tuna_fallback_control_plane_pressure={(postTunaFallbackControlPlanePressureActive ? 1 : 0)}; post_tuna_fallback_control_plane_pressure_remaining_ms={postTunaFallbackControlPlanePressureRemainingMs}; sample_window_ms={sampleWindowMs}");
            ...
            controlLastReceivedAgeMs >= ReceiveStallControlAgeThresholdMs)
        {
            ...
            if (fileTransferRuntimeActive)
            {
                ...
                if (!runtimeProtocolRepairGraceExhausted &&
                    !hasPostTunaFallbackLegAuthority &&
                    !regularV4ControlFeedbackPressureRecoveryNeeded)
                {
                    Log(
                ...
                else if (!runtimeProtocolRepairGraceExhausted && regularV4ControlFeedbackPressureRecoveryNeeded)
                {
                    ...
                    Log(
                        ...
                        $"connect_key={connectKey}; consecutive_zero_receive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; active_file_transfer_sessions={fileTransferActiveSessionCount}; active_file_transfer_runtime_sessions={fileTransferActiveRuntimeCount}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                        $"frames_sent_since_last={framesSentSinceLast}; control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; total_messages_received_since_last={totalMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; pressure_remaining_ms={regularV4ControlFeedbackPressureRemainingMs}; protocol_repair_grace_windows={ReceiveStallFileTransferProtocolRepairGraceWindows}; protocol_repair_grace_max_age_ms={ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs}");
                }
            }
                 ...
                 controlConsecutiveWindows >= ReceiveStallFastRequiredConsecutiveWindows)
        {
            if (bulkReceiveFreshForActiveFileTransfer)
            {
                ...
                Log(
                    ...
                    $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; total_messages_received_since_last={totalMessagesReceivedSinceLast}; " +
                    $"control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; regular_v4_control_feedback_pressure={(regularV4ControlFeedbackPressureActive ? 1 : 0)}; post_tuna_fallback_control_plane_pressure={(postTunaFallbackControlPlanePressureActive ? 1 : 0)}; sample_window_ms={sampleWindowMs}");
                if (options.ReceiveStallControlOnlyRecoveryEnabled &&
                    postTunaFallbackControlPlanePressureActive &&
                    pressureGraceExhausted)
                {
                    if (TryConsumePostTunaFallbackControlPlanePressureRecovery(
                            Stopwatch.GetTimestamp(),
                            out var consumedPressure,
                            out var consumedPressureRemainingMs))
                    {
                        Log(
                            "event=nkn_bridge_control_receive_recovery_allowed; reason=post_tuna_fallback_control_plane_pressure; " +
                            ...
                            $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; pressure_remaining_ms={consumedPressureRemainingMs}; protocol_repair_grace_windows={ReceiveStallFileTransferProtocolRepairGraceWindows}; protocol_repair_grace_max_age_ms={ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs}{FormatPostTunaFallbackControlPlanePressureFields(consumedPressure)}");
                    ...
                    {
                        Log(
                            $"event=nkn_bridge_control_receive_recovery_suppressed; reason=post_tuna_fallback_control_plane_pressure_budget_exhausted; " +
                            ...
                            $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; pressure_grace_exhausted={(pressureGraceExhausted ? 1 : 0)}; protocol_repair_grace_windows={ReceiveStallFileTransferProtocolRepairGraceWindows}; protocol_repair_grace_max_age_ms={ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs}{FormatPostTunaFallbackControlPlanePressureFields(postTunaFallbackControlPlanePressure)}");
                        EmitReceiveStallRecoveryDeferredLifecycle(
                            "post_tuna_fallback_control_plane_pressure_budget_exhausted",
                            PostTunaFallbackControlPlanePressureStallReason,
                            connectKey);
                        return;
                    }
                }
                else if (options.ReceiveStallControlOnlyRecoveryEnabled &&
                    regularV4ControlFeedbackPressureActive &&
                    pressureGraceExhausted)
                {
                    Log(
                ...
                else
                {
                    Log(
                        ...
                        $"connect_key={connectKey}; consecutive_control_zero_receive_windows={controlConsecutiveWindows}; active_file_transfer_sessions={fileTransferActiveSessionCount}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                        $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; regular_v4_control_feedback_pressure={(regularV4ControlFeedbackPressureActive ? 1 : 0)}; post_tuna_fallback_control_plane_pressure={(postTunaFallbackControlPlanePressureActive ? 1 : 0)}; pressure_grace_exhausted={(pressureGraceExhausted ? 1 : 0)}; protocol_repair_grace_windows={ReceiveStallFileTransferProtocolRepairGraceWindows}; protocol_repair_grace_max_age_ms={ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs}{FormatPostTunaFallbackControlPlanePressureFields(postTunaFallbackControlPlanePressure)}");
                    return;
                }
            }
                ...
                !options.ReceiveStallControlOnlyRecoveryEnabled)
            {
                Log(
                    ...
                    $"control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; sample_window_ms={sampleWindowMs}");
                Log(
                    "event=nkn_bridge_control_receive_recovery_suppressed; reason=filetransfer_control_only_recovery_disabled; " +
                    $"connect_key={connectKey}; consecutive_control_zero_receive_windows={controlConsecutiveWindows}; active_file_transfer_sessions={fileTransferActiveSessionCount}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                    $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}");
                return;
            }
        ...
        var protocolRepairGraceExhausted =
            ...
            Math.Max(controlLastReceivedAgeMs, bulkLastReceivedAgeMs) >= ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs;
        var postTunaFallbackUnprovenEscalation = false;
        var postTunaFallbackProofWindowExpiredForPressure = false;
        var regularV4ControlFeedbackPressureRecovery = false;
        if (fileTransferContextActive &&
            !explicitControlOnlyRecoveryOverride)
        {
            if (Volatile.Read(ref receiveStallRecoveryAwaitingReceiveProof) != 0 &&
                IsPostTunaFallbackProofSendWindowActive(
                    Stopwatch.GetTimestamp(),
                    out var proofWindowRemainingMs,
                    out var proofWindowReason,
                    out var proofWindowTrigger,
                    out var proofWindowSessionId))
            {
                if (postTunaFallbackControlPlanePressureActive &&
                    controlMessagesReceivedSinceLast == 0 &&
                    bulkMessagesReceivedSinceLast == 0 &&
                    qualifiedConsecutiveWindows >= ReceiveStallFastRequiredConsecutiveWindows)
                {
                    Interlocked.Exchange(ref postTunaFallbackProofSendWindowExpiresTick, 0);
                    postTunaFallbackProofWindowExpiredForPressure = true;
                    Log(
                        "event=nkn_bridge_post_tuna_fallback_proof_send_window_expired_for_pressure; " +
                        "reason=post_tuna_fallback_control_plane_pressure; " +
                        $"connect_key={connectKey}; stall_reason={stallReason}; consecutive_zero_receive_windows={qualifiedConsecutiveWindows}; all_zero_receive_consecutive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; " +
                        $"proof_window_reason={SanitizeLogToken(proofWindowReason)}; proof_window_trigger={SanitizeLogToken(proofWindowTrigger)}; session_id={SanitizeLogToken(proofWindowSessionId)}; proof_window_remaining_ms={proofWindowRemainingMs}; " +
                        $"active_file_transfer_sessions={fileTransferActiveSessionCount}; active_file_transfer_runtime_sessions={fileTransferActiveRuntimeCount}; active_post_tuna_fallback_runtime_sessions={activePostTunaFallbackRuntimeTransfers.Count}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                        $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; pressure_remaining_ms={postTunaFallbackControlPlanePressureRemainingMs}{FormatPostTunaFallbackControlPlanePressureFields(postTunaFallbackControlPlanePressure)}");
                }
                else
                {
                    Log(
                        ...
                        $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}");
                    EmitReceiveStallRecoveryDeferredLifecycle(
                        "post_tuna_fallback_proof_send_window",
                        stallReason,
                        connectKey);
                    return;
                }
            ...
            if (regularV4ControlFeedbackPressureRecoveryNeeded)
            {
                ...
                Log(
                    "event=nkn_bridge_receive_stall_recovery_protocol_repair_bypassed; reason=regular_v4_control_feedback_pressure; " +
                    $"connect_key={connectKey}; stall_reason={stallReason}; consecutive_zero_receive_windows={qualifiedConsecutiveWindows}; all_zero_receive_consecutive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; active_file_transfer_sessions={fileTransferActiveSessionCount}; active_file_transfer_runtime_sessions={fileTransferActiveRuntimeCount}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                    $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; pressure_remaining_ms={regularV4ControlFeedbackPressureRemainingMs}; protocol_repair_grace_windows={ReceiveStallFileTransferProtocolRepairGraceWindows}; protocol_repair_grace_max_age_ms={ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs}");
            }

            if (!postTunaFallbackProofWindowExpiredForPressure)
            {
                TryArmPostTunaFallbackUnprovenRecoveryEscalationForAutomaticStall(
                    "filetransfer_protocol_repair_only",
                    stallReason,
                    connectKey,
                    qualifiedConsecutiveWindows,
                    bulkConsecutiveWindows,
                    controlConsecutiveWindows,
                    Stopwatch.GetTimestamp());
            }

            if (postTunaFallbackProofWindowExpiredForPressure)
            {
                postTunaFallbackUnprovenEscalation = true;
                stallReason = "post_tuna_fallback_proof_window_unproven";
                requiresControlProof = true;
                requiresBulkProof = true;
                Log(
                    "event=nkn_bridge_receive_stall_recovery_protocol_repair_bypassed; reason=post_tuna_fallback_proof_window_unproven; " +
                    $"connect_key={connectKey}; stall_reason={stallReason}; consecutive_zero_receive_windows={qualifiedConsecutiveWindows}; all_zero_receive_consecutive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; active_file_transfer_sessions={fileTransferActiveSessionCount}; active_file_transfer_runtime_sessions={fileTransferActiveRuntimeCount}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                    $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; pressure_remaining_ms={postTunaFallbackControlPlanePressureRemainingMs}; protocol_repair_grace_windows={ReceiveStallFileTransferProtocolRepairGraceWindows}; protocol_repair_grace_max_age_ms={ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs}{FormatPostTunaFallbackControlPlanePressureFields(postTunaFallbackControlPlanePressure)}");
            }
            else if (TryConsumePostTunaFallbackUnprovenRecoveryEscalation(
                    "filetransfer_protocol_repair_only",
            ...
            {
                postTunaFallbackUnprovenEscalation = true;
                stallReason = "post_tuna_fallback_unproven_recovery_escalation";
                requiresControlProof = true;
                requiresBulkProof = true;
            }
            else if (regularV4ControlFeedbackPressureRecovery)
            {
                // Recovery is intentionally allowed below.
            }
            else if (protocolRepairGraceExhausted)
            {
                Log(
                    "event=nkn_bridge_receive_stall_recovery_protocol_repair_exhausted; " +
                    $"connect_key={connectKey}; stall_reason={stallReason}; consecutive_zero_receive_windows={qualifiedConsecutiveWindows}; all_zero_receive_consecutive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; active_file_transfer_sessions={fileTransferActiveSessionCount}; active_file_transfer_runtime_sessions={fileTransferActiveRuntimeCount}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
            ...
            {
                Log(
                    "event=nkn_bridge_receive_stall_recovery_suppressed; reason=filetransfer_protocol_repair_only; " +
                    $"connect_key={connectKey}; stall_reason={stallReason}; consecutive_zero_receive_windows={qualifiedConsecutiveWindows}; all_zero_receive_consecutive_windows={consecutiveWindows}; bulk_zero_receive_consecutive_windows={bulkConsecutiveWindows}; control_zero_receive_consecutive_windows={controlConsecutiveWindows}; active_file_transfer_sessions={fileTransferActiveSessionCount}; active_file_transfer_runtime_sessions={fileTransferActiveRuntimeCount}; recovery_count={Volatile.Read(ref receiveStallRecoveryCount)}; " +
                    $"control_messages_received_since_last={controlMessagesReceivedSinceLast}; bulk_messages_received_since_last={bulkMessagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; bulk_last_received_age_ms={bulkLastReceivedAgeMs}; protocol_repair_grace_windows={ReceiveStallFileTransferProtocolRepairGraceWindows}; protocol_repair_grace_max_age_ms={ReceiveStallFileTransferProtocolRepairGraceMaxAgeMs}");
                EmitReceiveStallRecoveryDeferredLifecycle(
                    "filetransfer_protocol_repair_only",
        ...
        var activeFileTransferRecoveryUnproven =
            fileTransferContextActive &&
            awaitingReceiveProof &&
            (requiresControlProofSnapshot || requiresBulkProofSnapshot);
        var regularV4UnprovenEscalation =
            !postTunaFallbackUnprovenEscalation &&
            regularV4ControlFeedbackPressureActive &&
            !regularV4ControlFeedbackPressureRecovery &&
            activeFileTransferRecoveryUnproven &&
            protocolRepairGraceExhausted &&
            TryConsumeRegularV4UnprovenRecoveryEscalation(
                "filetransfer_protocol_repair_only",
                stallReason,
                connectKey,
                ...
                bulkConsecutiveWindows,
                controlConsecutiveWindows);
        if (regularV4UnprovenEscalation)
        {
            stallReason = "regular_v4_unproven_recovery_escalation";
            requiresControlProof = true;
            requiresBulkProof = true;
        }
        ...
        var activeFileTransferShortCooldown = fileTransferContextActive;
        ...
        if (withinCooldown &&
            ...
            !regularV4UnprovenEscalation &&
            !regularV4ControlFeedbackPressureRecovery &&
            (!awaitingReceiveProof || activeFileTransferRecoveryUnproven))
        {
        ...
        if (withinCooldown && postTunaFallbackUnprovenEscalation)
        {
            var elapsedSinceLastRecoveryMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(lastRecoveryTick, nowTick).TotalMilliseconds);
            Log(
                "event=nkn_bridge_receive_stall_recovery_cooldown_bypassed; reason=post_tuna_fallback_unproven_escalation; " +
                $"connect_key={connectKey}; stall_reason={stallReason}; consecutive_zero_receive_windows={qualifiedConsecutiveWindows}; recovery_count={recoveryCount}; elapsed_since_last_recovery_ms={elapsedSinceLastRecoveryMs}; awaiting_receive_proof={(awaitingReceiveProof ? 1 : 0)}");
        }

        if (withinCooldown && regularV4UnprovenEscalation)
        {
            var elapsedSinceLastRecoveryMs = Math.Max(0, (long)Stopwatch.GetElapsedTime(lastRecoveryTick, nowTick).TotalMilliseconds);
            Log(
                "event=nkn_bridge_receive_stall_recovery_cooldown_bypassed; reason=regular_v4_unproven_escalation; " +
        ...
        if (Interlocked.CompareExchange(ref receiveStallRecoveryInProgress, 1, 0) != 0)
        {
            var enteredAfterClearingStaleGate = postTunaFallbackUnprovenEscalation &&
                                                TryClearCompletedUnprovenPostTunaFallbackRecoveryGate("post_tuna_fallback_peer_silence", "automatic_recovery_already_in_progress") &&
                                                Interlocked.CompareExchange(ref receiveStallRecoveryInProgress, 1, 0) == 0;
            if (!enteredAfterClearingStaleGate)
            {
                if (postTunaFallbackUnprovenEscalation &&
                    ArmPendingPostTunaFallbackUnprovenRecoveryEscalation(
                        "post_tuna_fallback_peer_silence",
                        "automatic_recovery_already_in_progress",
                        nowTick))
                {
                    Volatile.Write(ref postTunaFallbackUnprovenRecoveryEscalationUsedRecoveryCount, -1);
                    EmitReceiveStallRecoveryDeferredLifecycle(
        ...
        long bulkLastReceivedAgeMs)
    {
        ...
        try
        {
            ...
            var coreRequestedFileTransferRecovery = string.Equals(
                ...
                "core_filetransfer_request",
                StringComparison.Ordinal);
            var preferSoftFileTransferRecovery =
                coreRequestedFileTransferRecovery &&
                ShouldUseSoftFileTransferReceiveStallRecovery(stallReason);
            var useHardRestart =
                activeFileTransfer &&
                !preferSoftFileTransferRecovery &&
                (coreRequestedFileTransferRecovery ||
                 string.Equals(stallReason, RegularV4ControlFeedbackPressureStallReason, StringComparison.Ordinal) ||
                 attempt >= ReceiveStallActiveFileTransferHardRestartMinAttempt);

            if (preferSoftFileTransferRecovery)
            {
                Log(
                    "event=nkn_bridge_receive_stall_recovery_soft_for_filetransfer; " +
                    $"connect_key={connectKey}; stall_reason={stallReason}; attempt={attempt}; core_requested=1; " +
                    $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
            }

            async Task RunHardRestartRecoveryAsync(string trigger)
            {
                Log(
                    "event=nkn_bridge_receive_stall_recovery_hard_restart; " +
                    $"connect_key={connectKey}; stall_reason={stallReason}; attempt={attempt}; trigger={trigger}; core_requested={(coreRequestedFileTransferRecovery ? 1 : 0)}; " +
                    $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
            ...
            async Task RunSoftRecoveryConnectAsync()
            {
            ...
            if (useHardRestart)
            {
                await RunHardRestartRecoveryAsync("planned").ConfigureAwait(false);
            }
            else if (preferSoftFileTransferRecovery)
            {
                try
                {
                    await RunSoftRecoveryConnectAsync().ConfigureAwait(false);
                }
                catch (Exception ex) when (ex is not OperationCanceledException)
                {
                    Log(
                        ...
                        $"active_file_transfer_sessions={Math.Max(0, Volatile.Read(ref activeFileTransferDataSessions))}; active_file_transfer_runtime_sessions={activeFileTransferRuntimeTransfers.Count}");
                    await RunHardRestartRecoveryAsync("soft_failed").ConfigureAwait(false);
                }
            }
            else
            {
        ...
        {
            if (TryEmitActiveFileTransferReceiveStallRecoveryExhausted(connectKey, stallReason, attempt, ex))
            {
                return;
            }
        ...
        finally
        {
            ...
            Interlocked.Exchange(ref receiveStallRecoveryInProgress, 0);
            TrySchedulePendingPostTunaFallbackUnprovenRecoveryEscalation("recovery_finished");
        }
    }

    private bool TryEmitActiveFileTransferReceiveStallRecoveryExhausted(
        string connectKey,
        string stallReason,
        int attempt,
        Exception ex)
    {
        ...
        var exhaustedReason = $"{stallReason}_recovery_failed";
        NknRuntimeDiagnostics.SetLastError("nkn_bridge_receive_stall_recovery_exhausted");
        Log(
            "event=nkn_bridge_receive_stall_recovery_exhausted_for_filetransfer; " +
            $"connect_key={connectKey}; stall_reason={stallReason}; attempt={attempt}; reason={ex.GetType().Name}; message={SanitizeLogToken(ex.Message)}; " +
            $"active_file_transfer_sessions={activeDataSessionCount}; active_file_transfer_runtime_sessions={activeRuntimeCount}; exhausted_reason={SanitizeLogToken(exhaustedReason)}");
        EmitBridgeLifecycle(new BridgeLifecycleEvent(
            BridgeLifecycleEventKind.ReceiveStallRecoveryExhausted,
            StartMode: null,
            Pid: bridgeSupervisor.CurrentPid,
            ...
            ExitCode: null,
            ExitReasonKind: null,
            ExitReasonText: exhaustedReason));
        return true;
    }
        ...
        long bulkLastReceivedAgeMs)
    {
        ...
        Interlocked.Exchange(ref postTunaFallbackProofSendWindowExpiresTick, 0);
        ClearPostTunaFallbackUnprovenRecoveryEscalation("receive_resumed");
        Volatile.Write(ref regularV4UnprovenRecoveryEscalationUsedRecoveryCount, -1);
        var completedTick = Volatile.Read(ref receiveStallLastRecoveryCompletedTick);
        ...
        EmitBridgeLifecycle(new BridgeLifecycleEvent(
            ...
            MediaMessagesReceivedSinceLast: mediaMessagesReceivedSinceLast,
        ...
        try
        {
            while (!ct.IsCancellationRequested && !disposed)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (TrySuppressBridgePingTimeoutForActiveFileTransfer())
                    {
                        consecutiveMisses = 0;
                        continue;
                    }
    ...
    internal bool SuppressBridgePingTimeoutForActiveFileTransferForTests(
        long framesSentSinceLast,
        long messagesReceivedSinceLast,
        long controlLastReceivedAgeMs,
        long bulkLastReceivedAgeMs,
        long disconnectSignalCount,
        TimeSpan healthSummaryAge)
    {
        var ageTicks = (long)Math.Clamp(
            healthSummaryAge.TotalSeconds * Stopwatch.Frequency,
            0,
            long.MaxValue);
        Volatile.Write(ref lastBridgeTransportHealthSummaryTick, Stopwatch.GetTimestamp() - ageTicks);
        Volatile.Write(ref lastBridgeTransportHealthFramesSentSinceLast, Math.Max(0, framesSentSinceLast));
        Volatile.Write(ref lastBridgeTransportHealthMessagesReceivedSinceLast, Math.Max(0, messagesReceivedSinceLast));
        Volatile.Write(ref lastBridgeTransportHealthControlLastReceivedAgeMs, controlLastReceivedAgeMs);
        Volatile.Write(ref lastBridgeTransportHealthBulkLastReceivedAgeMs, bulkLastReceivedAgeMs);
        Volatile.Write(ref lastBridgeTransportHealthDisconnectSignalCount, Math.Max(0, disconnectSignalCount));
        return TrySuppressBridgePingTimeoutForActiveFileTransfer();
    }

    internal bool SuppressBridgePingTimeoutForActiveFileTransferAfterRecentSendForTests(
        NknBridgeChannel channel,
        long payloadBytes,
    ...
    {
        var ageTicks = (long)Math.Clamp(
            sendAge.TotalSeconds * Stopwatch.Frequency,
            0,
            long.MaxValue);
        Volatile.Write(ref lastBridgeSendFrameTick, Stopwatch.GetTimestamp() - ageTicks);
        Volatile.Write(ref lastBridgeSendFramePayloadBytes, Math.Max(0, payloadBytes));
        Volatile.Write(ref lastBridgeSendFrameSerializedBytes, Math.Max(0, serializedBytes));
        Volatile.Write(ref lastBridgeSendFrameChannel, (long)channel);
        return TrySuppressBridgePingTimeoutForActiveFileTransfer();
    }

    internal bool EmitActiveFileTransferReceiveStallRecoveryExhaustedForTests(string stallReason)
        => TryEmitActiveFileTransferReceiveStallRecoveryExhausted(
            "test_receive_stall_recovery",
            string.IsNullOrWhiteSpace(stallReason) ? "receive_stall_recovery" : stallReason.Trim(),
            1,
            new InvalidOperationException("test_receive_stall_recovery_failed"));

    private bool TrySuppressBridgePingTimeoutForActiveFileTransfer()
    {
        ...
        var recentActivityWindowMs = Math.Max(5000, (int)ScreenShareBridgeLogInterval.TotalMilliseconds * 3);
        var summaryTick = Volatile.Read(ref lastBridgeTransportHealthSummaryTick);
        var summaryAgeMs = ElapsedSinceTicksMilliseconds(summaryTick);
        var disconnectSignalCount = Volatile.Read(ref lastBridgeTransportHealthDisconnectSignalCount);
        if (summaryAgeMs is not null &&
            summaryAgeMs.Value <= recentActivityWindowMs &&
            disconnectSignalCount <= 0)
        {
            var framesSentSinceLast = Volatile.Read(ref lastBridgeTransportHealthFramesSentSinceLast);
            var messagesReceivedSinceLast = Volatile.Read(ref lastBridgeTransportHealthMessagesReceivedSinceLast);
            var controlLastReceivedAgeMs = Volatile.Read(ref lastBridgeTransportHealthControlLastReceivedAgeMs);
            var bulkLastReceivedAgeMs = Volatile.Read(ref lastBridgeTransportHealthBulkLastReceivedAgeMs);
            Log(
                "event=nkn_bridge_ping_timeout_filetransfer_recovery_suppressed; " +
                "reason=recent_bridge_health_summary; " +
                $"active_file_transfer_sessions={activeFileTransferSessionCount}; " +
                $"health_summary_age_ms={(long)summaryAgeMs.Value}; frames_sent_since_last={framesSentSinceLast}; " +
                $"messages_received_since_last={messagesReceivedSinceLast}; control_last_received_age_ms={controlLastReceivedAgeMs}; " +
                $"bulk_last_received_age_ms={bulkLastReceivedAgeMs}");
            return true;
        }

        var sendTick = Volatile.Read(ref lastBridgeSendFrameTick);
        var sendAgeMs = ElapsedSinceTicksMilliseconds(sendTick);
        if (sendAgeMs is not null &&
            sendAgeMs.Value <= recentActivityWindowMs)
        {
            var channel = (NknBridgeChannel)Volatile.Read(ref lastBridgeSendFrameChannel);
            var payloadBytes = Volatile.Read(ref lastBridgeSendFramePayloadBytes);
            var serializedBytes = Volatile.Read(ref lastBridgeSendFrameSerializedBytes);
            Log(
                "event=nkn_bridge_ping_timeout_filetransfer_recovery_suppressed; " +
                "reason=recent_bridge_send; " +
                $"active_file_transfer_sessions={activeFileTransferSessionCount}; " +
                $"send_age_ms={(long)sendAgeMs.Value}; send_channel={channel}; payload_bytes={payloadBytes}; serialized_bytes={serializedBytes}; " +
                $"health_summary_age_ms={(summaryAgeMs is null ? -1 : (long)summaryAgeMs.Value)}; disconnect_signal_count={disconnectSignalCount}");
            return true;
        }

        if (disconnectSignalCount > 0)
        {
            return false;
        }

        Log(
            "event=nkn_bridge_ping_timeout_filetransfer_recovery_not_suppressed; " +
            "reason=stale_bridge_health_and_send; " +
            $"active_file_transfer_sessions={activeFileTransferSessionCount}; " +
            $"send_age_ms={(sendAgeMs is null ? -1 : (long)sendAgeMs.Value)}; " +
            $"health_summary_age_ms={(summaryAgeMs is null ? -1 : (long)summaryAgeMs.Value)}; disconnect_signal_count={disconnectSignalCount}");
        return false;
    }
    ...
    private void EmitReceiveStallRecoveryDeferredLifecycle(
        string reason,
        string stallReason,
        string connectKey)
    {
        var reasonText =
            $"reason={SanitizeLogToken(reason)}:" +
            $"stall={SanitizeLogToken(stallReason)}:" +
            $"connect={SanitizeLogToken(connectKey)}";
        EmitBridgeLifecycle(new BridgeLifecycleEvent(
